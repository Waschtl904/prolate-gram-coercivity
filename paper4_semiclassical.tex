...
$\gamma$, $c$, $T$;
any factor of $n$ appearing in estimates is
therefore uniformly bounded and may be absorbed into
...
The two are related by differentiation in $\chi_n$
...
%% FIX 1: Self-contained lemma for ∫1/ω via energy differentiation
...
Differentiate $S_n^+$ from Lemma~\ref{lem:bohr-sommerfeld-pswf}
with respect to $\chi$:
\begin{equation}\label{eq:diff-chi}
  \frac{d}{d\chi} S_n^+(\chi)
...
\begin{equation}\label{eq:dSplus-dchi}
  ...
  = \frac{1}{2T^2}\,S_n^-(\chi).
\end{equation}
From \eqref{eq:BS}: $S_n^+(\chi_n) = \pi(n+\tfrac{1}{2}) + O(1/n)$.
Differentiating the quantization condition
$S_n^+(\chi_n(c)) = \pi(n+\tfrac{1}{2}) + O(1/n)$
with respect to $n$ (treating $\chi_n$ as a smooth function
of $n$ via the asymptotic $\chi_n \sim n^2 + O(n)$,
see \cite{OsipovRokhlinXiao}, Ch.~2):
\begin{equation}\label{eq:dchi-dn}
  \frac{dS_n^+}{d\chi}\cdot\frac{d\chi_n}{dn}
  = \pi + O\!\left(\frac{1}{n^2}\right).
\end{equation}
From $\chi_n \sim n^2$ we have $d\chi_n/dn \sim 2n$;
more precisely, $d\chi_n/dn = 2n + O(1)$ (see
\cite{OsipovRokhlinXiao}, Ch.~2, eq.~(2.17)).
Substituting \eqref{eq:dSplus-dchi} into \eqref{eq:dchi-dn}:
\[
  \frac{1}{2T^2}\,S_n^-\cdot(2n + O(1))
  = \pi + O\!\left(\frac{1}{n^2}\right).
\]
...
\eqref{eq:BS} (Lemma~\ref{lem:bohr-sommerfeld-pswf}),
the explicit formula \eqref{eq:wncl-def} for $\wncl$,
and the standard asymptotics $\chi_n \sim n^2$.
...
%% FIX 4: Gronwall integral ∫A dx = O(1) made explicit
...
and the Gronwall exponential factor satisfies:
\[
...
Step~4 applies Gronwall with zero initial data,
$O(1/n)$ forcing, and an exponential factor bounded by
