...
%% FIX 1: Lemma result uses original variable x in [-T,T];
%%        normalization correction t = x/T is explicit in the proof.
...
(Lemma~\ref{lem:chi-increment}, \eqref{eq:chi-spacing}), and
...
$\chi_{n+1} - \chi_n = 2n + O(1)$ is derived by direct
algebraic differencing of the known asymptotic expansion
of $\chi_n(c)$.
...
From \eqref{eq:r-ode} and Definition~\ref{def:wkb-amp} we have:
\[
  (\log r_n)' = G(x)\cos(2\theta_n(x)) + D(x),
\]
and
\[
...
\begin{align}
  ...
  &= G(x)\cos(2\theta_n(x)) + D(x) - D(x) + G(x) \notag \\
...
(with initial condition $\log(r_n/r_n^{\mathrm{WKB}})|_{x_0} = 0$)
and splitting $G(1+\cos(2\theta_n)) = G + G\cos(2\theta_n)$:
\[
  \log\frac{r_n(x)}{r_n^{\mathrm{WKB}}(x)}
  = \int_{x_0}^x G(t)\,dt
    + \int_{x_0}^x G(t)\cos(2\theta_n(t))\,dt.
\]
Since $(\log r_n^{\mathrm{WKB}})' = D - G$, integrating from $x_0$ to $x$
gives $\log r_n^{\mathrm{WKB}}(x) - \log r_n^{\mathrm{WKB}}(x_0)
= \int_{x_0}^x D\,dt - \int_{x_0}^x G\,dt$.
Combined with the initial condition, the $\int G\,dt$ term
cancels against the WKB definition, leaving:
