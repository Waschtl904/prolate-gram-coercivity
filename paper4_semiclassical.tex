...
%% FIX 1: constants explicitly depend on delta, gamma, c, T
All constants $C_{\delta,\gamma,c,T}$, $c_{\delta,\gamma,c,T}$
depend only on $\delta$, $\gamma$, $c$, $T$, and are independent
of $n$ and of the test function $f$.
For brevity we write $C_{\delta,\gamma}$ throughout, with the
understanding that $c$ and $T$ are always fixed parameters
on which all constants may depend.
...
\begin{equation}\label{eq:rho-cl}
  ...
  \quad x \in [-x_+(n), x_+(n)].
\end{equation}
The normalization $\int_{-x_+(n)}^{x_+(n)}\rhocl(x)\,dx = 1$
holds by definition.
By Lemma~\ref{lem:bohr-sommerfeld-pswf} below,
the denominator satisfies
\begin{equation}\label{eq:denom-BS}
  \int_{-x_+(n)}^{x_+(n)}\frac{dt}{\wncl(t)}
  = \pi\bigl(n+\tfrac{1}{2}\bigr) + O\!\left(\frac{1}{n}\right),
\end{equation}
where the $O(1/n)$ relative error propagates
to an $O(1/n)$ relative error in $\rhocl$ only through the
normalization constant; no mass is lost in the cutoff
$I_\delta \to [-x_+(n),x_+(n)]$ since
$\mathrm{supp}\,\rhocl = [-x_+(n),x_+(n)] \subset I_\delta$
exactly.
\end{definition}

\begin{remark}[Relation to~\eqref{eq:BS}]\label{rem:denom-vs-BS}
Note that~\eqref{eq:BS} (Lemma~\ref{lem:bohr-sommerfeld-pswf})
gives a quantization condition for $\int\wncl\,dx$,
whereas~\eqref{eq:denom-BS} is the analogous statement
for $\int 1/\wncl\,dx$.
Both follow from the same Liouville--WKB reduction;
the $O(1/n)$ error in~\eqref{eq:denom-BS} follows from the
same uniformity argument as in the proof of
...
\begin{proof}
The specialization to the PSWF operator
$(p\psin')' + \wncl(x)^2/p\cdot\psin = 0$
follows by reduction to Liouville normal form
\cite{Levitan}, Ch.~2.
After this reduction, the equation becomes a standard
Schr\"odinger-type equation with potential of size $O(n^2)$
and $C^2$-norm $O(n)$ on $I_\delta$,
so the standard WKB remainder bounds
(see \cite{OsipovRokhlinXiao}, Ch.~4)
apply directly.
The $O(1/n)$ error is uniform for $n \le \gamma N$ since
$\wncl \ge c_{\delta,\gamma}n$ (Lemma~\ref{lem:G-regularity}(i))
and $\|\wncl\|_{C^2(I_\delta)} = O(n)$,
so the WKB remainder bounds apply uniformly on $I_\delta$;
see also \cite{Levitan}, Ch.~2, for the Liouville reduction.
\end{proof}
...
%% FIX 2: Existence of J made explicit with formal construction
...
$c_{\delta,\gamma}n|I_\delta|/T \ge 2c_{\delta,\gamma}n$
over $I_\delta$,
so $\theta_n$ traverses at least
$M := \lfloor c_{\delta,\gamma}n/\pi\rfloor \ge 1$
complete periods on $I_\delta$ for $n \ge n_0(\delta,\gamma)$.

Each such period has length $\pi/\theta_n'(x) \le \pi T/(c_{\delta,\gamma}n)$
at any $x$ in that period.
On each interval of length $\pi/\theta_n'$ over which $\theta_n$
increases by exactly $\pi$, the function $\sin^2\theta_n$ equals
$1/4$ at exactly two interior points (where $\theta_n \equiv \pi/6$
and $\theta_n \equiv 5\pi/6 \pmod{\pi}$), and satisfies
$\sin^2\theta_n \ge 1/4$ on the middle third of the period.
Concretely, fix any complete period $[a,b] \subset I_\delta$
...
\[
  J := \theta_n^{-1}\!\left(
       ...
       \right) \subset [a,b]
\]
is well-defined (since $\theta_n$ is strictly increasing),
satisfies $\sin^2\theta_n(x) \ge 1/4$ for all $x \in J$, and has
length:
\[
  |J| = \int_{\theta_n(a)+\pi/6}^{\theta_n(a)+5\pi/6}
        \frac{d\theta}{\theta_n'(\theta_n^{-1}(\theta))}
      \ge \frac{2\pi/3}{\sup_{I_\delta}\theta_n'}
      \ge \frac{2\pi/3}{\,C_{\delta,\gamma}n\,}
      =: \frac{c_{\delta,\gamma}'}{n},
\]
using $\theta_n' \le C_{\delta,\gamma}n$ on $I_\delta$
(which follows from Lemma~\ref{lem:G-regularity}(i)
and \eqref{eq:theta-ode}).
...
\[
  r_n(x_0)^2
  \le \frac{4C_{\delta,\gamma}}{c_{\delta,\gamma}'}\cdot
      n\cdot\lambda_n.
\]
Since $n \le \gamma N = \gamma cT/\pi$,
the factor of $n$ is uniformly bounded by $\gamma cT/\pi$,
and is absorbed into the constant
$C_{\delta,\gamma,c,T} := 4C_{\delta,\gamma}\gamma cT/(\pi c_{\delta,\gamma}')$,
...
The explicit construction of $J$ via the inverse function
$\theta_n^{-1}$ is valid since $\theta_n$ is strictly
increasing on $I_\delta$ (Lemma~\ref{lem:phase-lower}).
\end{remark}
...
and Gronwall coefficient
$A(x) := 2D(x) + 2G(x)\cos(2\theta_n(x))$.
Here $|D| \le C_{\delta,\gamma}$ (Lemma~\ref{lem:G-regularity}(v))
dominates, giving $A(x) = O(1)$ uniformly on $I_\delta$;
the oscillatory term $2G\cos(2\theta_n)$ is of size $O(1/n)$
and does not affect the exponential growth factor in Gronwall,
which is governed by $D(x)$ alone.
...
(governed by $D$, not by the oscillatory $G$-term).
...
\eqref{eq:denom-BS}, and Definition~\ref{def:rho-cl}.
%% FIX 4: Explicit normalization-cutoff compatibility
Note that the normalization of $\rhocl$ is taken over
$[-x_+(n), x_+(n)]$, and by~\eqref{eq:denom-BS}
this normalization agrees with
$\tfrac{1}{2}\int r_n^{\mathrm{WKB},2}$
up to a relative error $O(1/n)$.
In particular, no additional mass is lost in the cutoff
$I_\delta \to [-x_+(n), x_+(n)]$,
since $\mathrm{supp}\,\rhocl = [-x_+(n),x_+(n)] \subset I_\delta$
exactly (Definition~\ref{def:rho-cl}).
\end{proof}
...
$\|b_n'\|_{L^\infty} = O(1/n)$.
We apply Corollary~\ref{cor:osc-slowly-varying}
with $a = f$ and $b_n = r_n^2/r_n(x_0)^2$:
\[
  \left|\int_{I_\delta}fr_n^2\cos(2\theta_n)\right|
  = r_n(x_0)^2\left|\int_{I_\delta}fb_n\cos(2\theta_n)\right|
  \le \frac{C_{\delta,\gamma}\,r_n(x_0)^2
            \,\|f\|_{C^1}}{n}
  \le \frac{C_{\delta,\gamma}\lambda_n(\|f\|_{L^\infty}
            + T\|f'\|_{L^\infty})}{n},
\]
using Lemma~\ref{lem:pointwise-control}~\eqref{eq:pointwise-lambda}
in the last step.
