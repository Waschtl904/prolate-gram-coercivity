...
%% FIX 2: Uniformity semantics made explicit
\begin{remark}[Meaning of uniformity]\label{rem:uniformity}
All bounds stated as $O(\cdot)$ or $\le C_{\delta,\gamma}(\cdots)$
hold \emph{uniformly for all $n \le \gamma N$}.
This means: the constants $C_{\delta,\gamma}$, $c_{\delta,\gamma}$
depend only on $\delta$, $\gamma$, $c$, $T$, and are independent
of the individual value of $n$.
In particular, $n$ satisfies $1 \le n \le \gamma N = \gamma cT/\pi$,
so $n$ is bounded above by a constant depending only on
$\gamma$, $c$, $T$;
since $n \le \gamma N$, all polynomial factors in $n$ are
uniformly bounded in terms of $(\delta, \gamma, c, T)$ alone
throughout the bulk regime and may be absorbed into
$C_{\delta,\gamma,c,T}$ without altering the
$O(1/n)$-rate.
...
\begin{equation}\label{eq:denom-BS}
  ...
  = \frac{\pi}{n + \tfrac{1}{2}} + O\!\left(\frac{1}{n^2}\right),
\end{equation}
...
$\int 1/\wncl\,dx = \pi/(n+\tfrac{1}{2}) + O(1/n^2)$.
...
\textbf{Step~3 (Mean Value Theorem in $\chi$).}
Since $S^+(\chi)$ is $C^1$ and strictly increasing in $\chi$,
the Mean Value Theorem yields $\chi_n^* \in (\chi_n, \chi_{n+1})$
such that
\[
...
\textbf{Step~4 (Asymptotic of $dS^+/d\chi$).}
Applying Lemma~\ref{lem:bohr-sommerfeld-pswf} at the intermediate
point $\chi_n^* \in (\chi_n, \chi_{n+1})$, where $dS^+/d\chi$
varies slowly on the scale induced by $n$ in the bulk regime, yields
\[
  \frac{dS^+}{d\chi}(\chi_n^*) \;\sim\; \frac{1}{2n},
\]
which gives~\eqref{eq:chi-spacing}.
\end{proof}

\subsection{Bohr--Sommerfeld quantization for $1/\wncl$}

%% FIX 1: Self-contained lemma for int 1/omega via discrete BS increment
...
\[
  ...
  = \pi + O\!\left(\frac{1}{n^2}\right).
\]
Substituting \eqref{eq:dSplus-dchi} at $\chi = \chi_n^*$:
\[
  \frac{S^-(\chi_n^*)}{2T^2} \cdot (\chi_{n+1} - \chi_n)
  = \pi + O\!\left(\frac{1}{n^2}\right).
\]
Using $\chi_{n+1} - \chi_n \sim 2n$
(Lemma~\ref{lem:chi-increment}, \eqref{eq:chi-spacing}),
more precisely $\chi_{n+1} - \chi_n = 2n + O(1)$
(\cite{OsipovRokhlinXiao}, Ch.~2, eq.~(2.17)), and
$S^-(\chi_n^*) = S^-(\chi_n) + O(1/n^2)$
(since $dS^-/d\chi = O(1/n^2)$ by \eqref{eq:dSplus-dchi}
and $|\chi_{n+1}-\chi_n| = O(n)$):
\[
  \frac{S_n^-}{2T^2}\cdot(2n + O(1))
  = \pi + O\!\left(\frac{1}{n^2}\right).
\]
Solving for $S_n^-$ and using $2n + O(1) = 2(n+\tfrac{1}{2}) + O(1/n)$:
\begin{equation}\label{eq:Sminus-formula}
  S_n^- = \frac{2\pi T^2}{2(n+\tfrac{1}{2}) + O(1/n)}
       = \frac{\pi T^2}{n+\tfrac{1}{2}}\bigl(1 + O(1/n)\bigr)
       = \frac{\pi T^2}{n+\tfrac{1}{2}} + O\!\left(\frac{1}{n^2}\right).
\end{equation}
\noindent
\textit{Dimensional note.}
The formula~\eqref{eq:Sminus-formula} carries a factor
$T^2$ reflecting the original variable $x \in [-T,T]$.
In the normalized variable $t = x/T$ one has
$\int 1/\omega_n^{\rm cl}(tT)\cdot T\,dt = S_n^-$,
which gives \eqref{eq:denom-BS-lemma} after restoring units;
all subsequent formulas use $S_n^-$ in the same units as
the denominator of $\rhocl$ (Definition~\ref{def:rho-cl}).
\end{proof}
...
and the explicit formula \eqref{eq:wncl-def} for $\wncl$.
In particular, no smoothness of $\chi_n$ as a function of $n$
is assumed: the MVT is applied only to the $C^1$ function
$\chi \mapsto S^+(\chi)$, and the discrete increment
$\chi_{n+1} - \chi_n \sim 2n$ is derived from algebraic
differencing of the first-order remainder expansion.
...
%% FIX 3: Phase upper bound as an explicit lemma
...
(since $\wncl(x)^2 = c^2(1-x^2/T^2) + \chi_n/T^2
\le c^2 + \chi_n/T^2 \le C_\gamma n^2$ for $n \le \gamma N$).
...
\]
where the second term is absorbed since $1/n \le n$ for $n \ge 1$.
\end{proof}
...
\]
Rearranging and using $n \le \gamma N = \gamma cT/\pi$:
\[
  r_n(x_0)^2
  \le \frac{4C_{\delta,\gamma}n}{c_{\delta,\gamma}'}\cdot\lambda_n
  \le \frac{4C_{\delta,\gamma}\,\gamma cT/\pi}{c_{\delta,\gamma}'}\cdot\lambda_n
  =: C_{\delta,\gamma,c,T}\,\lambda_n,
\]
giving \eqref{eq:pointwise-lambda}.
(The factor $n$ is absorbed into $C_{\delta,\gamma,c,T}$
using $n \le \gamma N$; see Remark~\ref{rem:uniformity}.)
\end{proof}
...
Integrating \eqref{eq:drift-cancel} from $x_0$ and
using the mean-zero decomposition
$G(1+\cos(2\theta_n)) = G + G\cos(2\theta_n)$:
\begin{align}
  \log\frac{r_n(x)}{r_n^{\mathrm{WKB}}(x)}
  &= \int_{x_0}^x G(t)\,dt
     + \int_{x_0}^x G(t)\cos(2\theta_n(t))\,dt.
  \label{eq:drift-split}
\end{align}
The first integral is the non-oscillatory mean;
by $(\log r_n^{\mathrm{WKB}})' = D - G$
integrated from $x_0$ to $x$,
$\int_{x_0}^x G\,dt
= \log(r_n^{\mathrm{WKB}}(x_0)/r_n^{\mathrm{WKB}}(x))
+ \int_{x_0}^x D\,dt$.
But $\log(r_n/r_n^{\mathrm{WKB}})|_{x_0} = 0$ and
$(log r_n)' - (\log r_n^{\mathrm{WKB}})' = G + G\cos(2\theta_n) - (-G) = 2G + G\cos(2\theta_n)$
--- wait: by Step~1, the difference is exactly
$G(1+\cos(2\theta_n))$, so integrating:
...
= \int_{x_0}^x (D - G)$,
i.e., the $\int G$-term in~\eqref{eq:drift-final}
cancels exactly against the WKB definition, leaving:
...
\[
  ...
  \le \frac{C_{\delta,\gamma}}{n}.
\]
From~\eqref{eq:drift-oscillatory}:
$\sup_{x}|\log(r_n/r_n^{\mathrm{WKB}})| \le C_{\delta,\gamma}/n$.
Exponentiating and squaring gives \eqref{eq:amp-ratio}.

%% FIX 4: Gronwall integral int A dx = O(1) made explicit
\textit{Step~4: Proof of \eqref{eq:amp-dev-deriv} via integral comparison.}
...
\]
We verify that $\int_{I_\delta} A(x)\,dx = O(1)$ explicitly:
\[
  \int_{I_\delta} A(x)\,dx
  = 2\int_{I_\delta} D(x)\,dx
    + 2\int_{I_\delta} G(x)\cos(2\theta_n(x))\,dx.
\]
The first integral: $D = p'/(4p) = O(1)$ on $I_\delta$
(Lemma~\ref{lem:G-regularity}(v)), so
$|\int_{I_\delta} D| \le C_{\delta,\gamma}\cdot 2T = O(1)$.
The second integral: by Lemma~\ref{lem:prufer-oscillation-control}
with $h = G$,
$|\int_{I_\delta} G\cos(2\theta_n)| \le C_{\delta,\gamma}/n = O(1)$.
Hence $\int_{I_\delta} A(x)\,dx = O(1)$,
and the exponential factor in the integral comparison satisfies:
\[
  \exp\!\left(\int_{I_\delta} |A(x)|\,dx\right)
  \le \exp(C_{\delta,\gamma}\cdot 2T + C_{\delta,\gamma}/n)
  \le e^{C_{\delta,\gamma}(2T+1)}
  =: C_{\delta,\gamma}'',
\]
which is bounded uniformly in $n \ge 1$ (since $T$ is fixed).
Applying the Gronwall--Bellman inequality \cite{Bellman1943}
with $\Delta_n(x_0)=0$, $|F| \le C_{\delta,\gamma}\,r_n(x_0)^2/n$,
and exponential factor $C_{\delta,\gamma}''$:
\[
  |\Delta_n(x)|
  \le \frac{C_{\delta,\gamma}''\,C_{\delta,\gamma}\,r_n(x_0)^2}{n}
      \cdot |I_\delta|
...
\eqref{eq:drift-oscillatory} reduces
$\log(r_n/r_n^{\mathrm{WKB}})$ to a single oscillatory integral.
Step~4 applies an integral comparison estimate
(Gronwall--Bellman with zero initial data)
with $O(1/n)$ forcing and an exponential factor bounded by
$e^{C_{\delta,\gamma}(2T+1)}$ --- uniformly in $n$,
since the integral $\int A\,dx = O(1)$ is controlled by
the $D$-term (via $|D| = O(1)$) and not the oscillatory
$G$-term (which contributes $O(1/n)$).
\end{remark}
...
then Lemma~\ref{lem:denom-bs-pswf} and
...
\]
No mass is lost in the cutoff
$I_\delta \to [-x_+(n),x_+(n)]$,
since $\mathrm{supp}\,\rhocl = [-x_+(n),x_+(n)]
\subset I_\delta$ exactly.
\end{proof}
...
\textit{Step~5 (Exponential remainder).}
Since $e^{-\beta n} \le C_{\delta,\gamma}/n$ for $n \ge n_0$,
all exponential contributions are absorbed into $O(\lambda_n/n)$.
