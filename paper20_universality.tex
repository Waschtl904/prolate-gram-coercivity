...
see Remark~\ref{rem:cadmissible}).
...
All constants appearing in this paper are $c$-admissible:
\begin{itemize}[nosep]
  \item $C_0$, $C_\infty$: operator-intrinsic, $c$-free by H1--H2.
  \item $C_\sigma = \frac{|B_2|}{12}\sigma(\sigma-1)$:
    depends only on the exponent $\sigma$, not on $K$ or $c$
    (Lemma~\ref{lem:trunc_universal}, eq.~\eqref{eq:EM_remainder}).
  \item $C_{\rm lin} = \frac{2MC_\infty C_0}{1+\gamma}$,
    $C_{\rm quad} = \frac{MC_0^2}{1+2\gamma}$:
    products of $c$-admissible quantities.
  \item $C_2(\alpha) = \sup_{K\geq 0}\|\BK\|^2 e^{\alpha K}$:
    operator-intrinsic; depends only on $\cAg\in\mathfrak{T}_\gamma$
    and the decay rate $\alpha$, not on $c$
    (Lemma~\ref{lem:gamma_agnostic}(v)).
    In particular, $c$ is only an approximation parameter
    in this paper, not a spectral parameter of $\cAg$.
\end{itemize}
...
\item[(v)] $\|\BK\|\leq\Ctwo^{1/2}e^{-\alpha K/2}$,
  ...
  This supremum depends only on $\cAg\in\mathfrak{T}_\gamma$
  and the decay rate $\alpha$;
  it is independent of the approximation parameter $c$,
  since $\BK$ is determined by the spectral data of $\cAg$ alone.
...
\[
  ...
  C_{\rm quad} = \frac{MC_0^2}{1+2\gamma},
\]
both $c$-admissible (Remark~\ref{rem:cadmissible}),
...
$c$-admissible constants as stated.
\end{proof}
...
not a uniform contraction in $c$:
$|\Phi_c'(K^*(c))|=\frac{2\gamma}{\alpha K^*(c)}\to 0$.
For each fixed $c$, Lipschitz constant $\lambda(c)<\tfrac{1}{2}$
on $[c_1\log c, c_2\log c]$.
Unique fixed point $=K^*(c)$ by uniqueness from~(i),
not by contraction strength.
Purpose: certify regime self-consistency;
...
\end{equation}
Relative error $O(\log\log c/\log c) = o(1)$;
the asymptotic order of each term in~\eqref{eq:fixed_param_bound}
is preserved.
