...
ensures that the realization map
$\\mathfrak{R}\\colon \\{\\text{analytic bounds}\\} \\to \\mathcal{S}/\\!\\sim_{\\mathcal{S}}$
(Definition~\\ref{def:realization_map}) is $c$-uniformly stable,
so that leading-order bounds project to fixed equivalence classes
under $c$-rescaling; without S2 this stability fails and the
balance fixed-point argument collapses (Lemma~\\ref{lem:S2_knockdown});
...
\\begin{itemize}[nosep]
  ...
  \\item \\textbf{(S2) = EM structural property:}
    ensures that the realization map $\\mathfrak{R}$
    (Definition~\\ref{def:realization_map}) is $c$-uniformly stable:
    ...
    what S2 protects is the stability of the map from bounds to
    those index pairs.
    Without S2, the realization map $\\mathfrak{R}$ is not
    $c$-uniformly stable, the comparison $\\Phi_1 \\succ \\Phi_2$
    as applied to the actual bounds ceases to be $c$-uniform,
    ...
    in the WKB power-sum class; other summation methods
    satisfying leading-power extraction, uniform remainder control,
    and scaling invariance of coefficients serve equally.
...
realization-map stability (S2),\\quad
...
\\]
This is the lexicographic order on $(s,t)\\in\\R^2$; it is defined
purely on index pairs, independently of any function representation.
The preorder on $\\mathcal{S}$ is unconditionally well-defined.
\\end{definition}

%% NEW: explicit realization map as intermediate layer.
%% This is where S2 sits: not on the preorder (which is always
%% well-defined), but on the stability of the map from
%% analytic bounds to equivalence classes.
...
equivalence class to which the bound $b$ projects.

\\emph{Role of S2.}
S2 is precisely the condition that ensures $\\mathfrak{R}$ is
$c$-uniformly stable for all bounds arising in
Lemma~\\ref{lem:trunc_universal}:
the prefactor $A(K,c) = C(K,c)$ is $c$-admissible,
so $b(K^*(c),c)\\sim_{\\mathcal{S}}\\phi_{s,t}$ holds with
ratio bounds independent of all external parameters,
and $\\mathfrak{R}$ maps to a fixed class for all $c$.
Without S2, $C(K,c)$ may grow with $c$, the ratio
$b(K^*(c),c)/\\phi_{s,t}(c)$ is not bounded uniformly,
and $\\mathfrak{R}(b)$ is not well-defined as a single
equivalence class.
\\end{definition}

\\begin{remark}[Preorder vs.\\ realization map: strict separation]
\\label{rem:scale_admissibility}
The preorder $\\succ$ on $\\mathcal{S}$ lives on the \\emph{target}
of $\\mathfrak{R}$: it compares index pairs $(s_1,t_1)$ vs.\\ $(s_2,t_2)$
and is unconditionally well-defined.
S2 governs the \\emph{source} side: it ensures that each analytic
bound has a well-defined, $c$-stable image under $\\mathfrak{R}$.
The two layers are logically independent:
one can have a well-defined preorder on $\\mathcal{S}$ and
simultaneously have analytic bounds whose realization map
is not $c$-uniformly stable (precisely the situation without S2).
\\end{remark}
...
\\begin{itemize}[nosep]
  ...
  \\item \\textbf{S2 (EM):} enters at rem:EM\\_growth as the mechanism
    ensuring $c$-uniform stability of the realization map $\\mathfrak{R}$
    (Definition~\\ref{def:realization_map}).
    Lemma~\\ref{lem:S2_knockdown} isolates what fails without S2:
    not the preorder, but $\\mathfrak{R}$.
...
\\begin{itemize}[nosep]
  ...
  \\item \\textbf{S2/EM} enters in two places:
    (a) predecessor of lem:trunc\\_univ
      (certifies $c$-admissibility of EM coefficients, ensuring
  ...
  \\item lem:H1\\_rigidity and lem:S2\\_knockdown certify
    exponent-independence and realization-map stability respectively;
    both are sibling inputs to thm:obstruction.
...
\\begin{enumerate}[nosep,label=(\\roman*)]
  ...
  \\item \\textbf{Realization-map stability:}
    ...
    $c$-admissible, ensuring that each bound has a well-defined,
...
\\begin{lemma}[S2 knockdown: failure of realization-map stability]
\\label{lem:S2_knockdown}
Assume that S2 is replaced by a summation method yielding only
...
\\end{equation}
where the implicit constant in $O_*(\\cdot)$ is not $c$-admissible
(the prefactor $A(K,c)$ absorbing $R_\\gamma^*$ is not
$c$-uniformly bounded; it may grow with $c$ or depend on $K$).
Then:
\\begin{itemize}[nosep]
  ...
  \\item[\\textup{(ii)}] \\textbf{Comparison of bounds loses $c$-uniformity.}
    The preorder $\\succ$ on $\\mathcal{S}$ is defined on index pairs
    and remains unconditionally well-defined.
    However, comparing the \\emph{bounds} $\\Phi_1$ and $\\Phi_2$
    via $\\succ$ requires that each bound has a stable image under
    $\\mathfrak{R}$; by~(i), $\\Phi_1$'s image is not stable.
    Hence the statement ``$\\Phi_1\\succ\\Phi_2$ as bounds''
    is not $c$-uniform: it may hold for some values of $c$
    and fail for others as $A(K,c)$ grows.
  ...
  \\item[\\textup{(iii)}] \\textbf{Balance fixed-point loses its basis.}
    ...
    which is established via
    Proposition~\\ref{prop:substitution_stability}(ii)
    using both a $c$-uniform upper and lower bound on
    $\\Psi/\\phi_{\\beta,1+\\gamma}$.
    The upper bound requires $\\mathfrak{R}(\\Phi_1)=[\\phi_{\\beta,1+\\gamma}]$
    to hold $c$-uniformly; by~(i) this fails without S2,
    and the $\\asymp$-conclusion has no valid basis.
\\end{itemize}

\\emph{Upshot.}
The preorder $\\succ$ on $\\mathcal{S}$ is not affected by the absence of S2:
it is defined on index pairs and remains well-defined.
What S2 protects is the realization map $\\mathfrak{R}$:
S2 is the condition that every analytic bound arising in
Lemma~\\ref{lem:trunc_universal} has a $c$-uniformly stable image
under $\\mathfrak{R}$, and hence a well-defined position in
the preorder.
The quantifier shift is:
...
\\textup{(ii)} Comparing bounds via $\\succ$ requires each bound
...
the upper bound is $\\mathfrak{R}(\\Phi_1) = [\\phi_{\\beta,1+\\gamma}]$
...
see Definition~\\ref{def:realization_map}):
...
\\begin{itemize}[nosep,leftmargin=1.5em]
  ...
  \\item Decomposition stability; balance fixed-point;
    H1 exponent rigidity; non-tautology; method/operator scope;
