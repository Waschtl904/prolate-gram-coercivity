...
All implied constants are $c$-admissible
(operator-intrinsic or deriving from H1--H2 with no $c$-dependence;
see Remark~\ref{rem:cadmissible} and Lemma~\ref{lem:cadm_closure}).
...
There are three sources of $c$-admissibility in this paper:
\begin{itemize}[nosep]
  \item \textbf{Axiomatic:} $C_0$, $C_\infty$ are declared $c$-free
    by H1 and H2 respectively (operator-intrinsic).
  \item \textbf{Analytic:} $C_\sigma = \frac{|B_2|}{12}\sigma(\sigma-1)$
    depends only on the Euler--Maclaurin exponent $\sigma$,
    not on $K$ or $c$ (Lemma~\ref{lem:trunc_universal},
    eq.~\eqref{eq:EM_remainder}).
  \item \textbf{Derived:} products and finite sums of $c$-admissible
    constants are $c$-admissible
    (Lemma~\ref{lem:cadm_closure}).
    In particular: $C_{\rm lin} = \frac{2MC_\infty C_0}{1+\gamma}$,
    $C_{\rm quad} = \frac{MC_0^2}{1+2\gamma}$,
    $C_{\sigma,1}$, $C_{\sigma,2}$ all fall under this case.
  \item \textbf{Operator-intrinsic:}
    $C_2(\alpha) = \sup_{K\geq 0}\|\BK\|^2 e^{\alpha K}$
    is $c$-admissible because $\BK$ is defined at the level of
    the operator $\cAg$ (spectral projections of $\cAg$ alone,
    no truncation or approximation parameter involved);
    see Lemma~\ref{lem:gamma_agnostic}(v) and
...
The class of $c$-admissible constants
(Definition: independent of the approximation parameter $c$)
is closed under finite products and finite sums.
\end{lemma}
...
\begin{itemize}[nosep]
  \item \textbf{Operator estimates (fixed-parameter regime):}
    $c$ appears only as a scaling factor in the operator bounds
    (e.g.\ $c^{-\beta}$ in Theorem~\ref{thm:universal_rate}).
  ...
  \item \textbf{Domain restriction (diagonal regime):}
    ...
    and in the definition of $K^*(c)$ via the fixed-$c$ FOC.
...
The transition from fixed-parameter to diagonal is performed
by exactly one lemma (Lemma~\ref{lem:diagonal_sub}),
which is the sole point where both roles interact.
\end{remark}
...
\begin{itemize}[nosep]
  ...
  \item Lemma~\ref{lem:diagonal_sub} is the unique node
    ...
    bound (with $c$-admissible constant from
...
The parameter $c$ in this paper is an approximation
scale parameter, not a spectral parameter of $\cAg$;
...
operator norms of spectral remainder terms) are therefore $c$-free
at the level of definition, before any approximation is applied.
\end{definition}
...
where the $u_j$ are eigenfunctions of $\cAg$
...
$\BK$ is defined as the spectral remainder of $\cAg$
after truncation at index $K$:
it is a functional of the spectral projections
$\{P_j : j > K\}$ of $\cAg$ alone.
No approximation parameter $c$ enters the definition of $\BK$.
In particular, $C_2(\alpha) = \sup_{K\geq 0}\|\BK\|^2 e^{\alpha K}$
is a constant of the operator $\cAg$ and the decay rate $\alpha$;
...
\item[(v)] $\|\BK\|\leq\Ctwo^{1/2}e^{-\alpha K/2}$,
  where
  $C_2(\alpha) := \sup_{K\geq 0}\|\BK\|^2 e^{\alpha K}$.
  By Remark~\ref{rem:BK_definition}, $C_2(\alpha)$ is $c$-admissible:
  it depends only on $\cAg\in\mathfrak{T}_\gamma$
  and the decay rate $\alpha$.
...
(products of $c$-admissible quantities from H1--H2),
such that for all $K\geq 1$, $c\geq c_0$:
\begin{align}
  \normh{\AKlin}{}
...
\end{align}
where $C_{\sigma,1} = \tfrac{1}{2}C_0 C_\infty$
and $C_{\sigma,2} = \tfrac{1}{2}C_0^2$
are also $c$-admissible (Lemma~\ref{lem:cadm_closure}).
\end{lemma}
...
Multiplying by the $c$-admissible prefactors from H1--H2
yields~\eqref{eq:AKlin}--\eqref{eq:AKquad} with explicit
$c$-admissible constants; closure under products by
Lemma~\ref{lem:cadm_closure}.
\end{proof}
...
hence $\exists c_0$: $g(c)\leq\alpha/4$, giving $K^*\leq c_2\log c$.
\emph{Lower.}
$F'(c_1\log c;c)>0$ for large $c$;
...
for all large $c$.
For each fixed $c$, $F(\,\cdot\,;c)$ is strictly convex on this window:
\[
  F''(K;c)
...
\]
$F''(K;c)\to 0$ as $c\to\infty$ for fixed $K$;
no uniform-in-$c$ convexity is claimed or needed.
Strict convexity at each fixed $c$, on the sandwich window,
...
\end{equation}
Derivation (no circularity; DAG: Remark~\ref{rem:dag_nocycle}):
\begin{itemize}[nosep]
  \item Step~1: $K^*\to\infty\Rightarrow\gamma\log K^*/K^*\to 0
...
(non-uniform, pointwise in $c$); role of $\lambda(c)$.}
$\Phi_c(K)=\frac{2\beta}{\alpha}\log c+\frac{2\gamma}{\alpha}\log K+C_1$.
For each fixed $c$, the Lipschitz constant of $\Phi_c$ on
$[c_1\log c, c_2\log c]$ satisfies
$\lambda(c) := |\Phi_c'(K^*(c))| = \frac{2\gamma}{\alpha K^*(c)} \to 0$.
Hence $\lambda(c) < \tfrac{1}{2}$ for large $c$,
certifying that the fixed-point iteration is locally contractive
at each fixed $c$ (regime self-consistency).
\emph{Remark on role:}
The unique fixed point $K^*(c)$ is established by part~(i)
(strict convexity / uniqueness of zero of $F'$),
not by the Banach fixed-point theorem.
The bound $\lambda(c)<\tfrac{1}{2}$ is not used for existence;
it certifies only that the iteration map $\Phi_c$
does not amplify perturbations at the solution,
i.e.\ that the iterated expansion~\eqref{eq:kopt_iterated}
is self-consistent.
...
\begin{proof}
MVT; for large $c$: $|K_1-K_2|\leq 4D/\alpha=O(1)$.
\end{proof}
...
\emph{Hypothesis.}
There exist $c$-admissible constants
$C_{\rm lin}, C_{\rm quad}, C_{\rm tail}$
(Remark~\ref{rem:cadmissible}, Lemma~\ref{lem:cadm_closure};
from Lemmas~\ref{lem:trunc_universal} and~\ref{lem:gamma_agnostic}(v))
such that for all $K\geq K_0$, $c\geq c_0$:
...
\emph{Slowly-varying substitution stability (uniform in $\gamma$).}
Let $h(c) := K^*(c)\big/\bigl(\tfrac{2\beta}{\alpha}\log c\bigr)$.
By~\eqref{eq:kopt_iterated}:
\begin{equation}\label{eq:h_rate}
  h(c) \to 1,
  \qquad
  |h(c) - 1| = O\!\left(\frac{\log\log c}{\log c}\right).
\end{equation}
For every fixed $\gamma > 0$, by the MVT
$(1+u)^{1+\gamma} = 1 + (1+\gamma)\theta^{\gamma} u$
for some $\theta$ between $1$ and $1+u$,
with $|u| = |h(c)-1| \leq \tfrac{1}{2}$ for large $c$:
the implied $O$-constant in
\begin{equation}\label{eq:hgamma}
  h(c)^{1+\gamma} = 1 + O\!\left(\frac{\log\log c}{\log c}\right)
\end{equation}
equals $(1+\gamma)\cdot\sup_{|u|\leq 1/2}(1+u)^\gamma \leq (1+\gamma)\cdot 2^\gamma$,
which is bounded on every compact subset $\gamma\in[\gamma_0,\gamma_1]$
with $0 < \gamma_0 \leq \gamma_1 < \infty$.
Hence~\eqref{eq:hgamma} and
\begin{equation}\label{eq:Kstar_power}
  (K^*(c))^{1+\gamma}
  = \Bigl(\tfrac{2\beta}{\alpha}\log c\Bigr)^{1+\gamma}
  \Bigl(1 + O\Bigl(\tfrac{\log\log c}{\log c}\Bigr)\Bigr)
\end{equation}
hold uniformly in $\gamma$ on compact subsets of $(0,\infty)$.
For fixed $\gamma$, relative error $O(\log\log c/\log c) = o(1)$.

\emph{Conclusion.}
At $K=K^*(c)$ (Lemma~\ref{lem:kopt_variational}(ii)),
as $c\to\infty$:
\begin{itemize}[nosep]
  ...
  \item[\textup{(ii)}] $K^*(c)^{1+2\gamma}c^{-2\beta}
    ...
    ratio $= O((\log c)^\gamma c^{-\beta})\to 0$.
  \item[\textup{(iii)}] $C_{\rm tail}\,e^{-\alpha K^*(c)/2}
    \prec c^{-\beta}(\log c)^{1+\gamma}$:
    ratio $= O((\log c)^{-(1+2\gamma)})\to 0$.
  \item[\textup{(iv)}] $\normh{\cK-\cKA}{}
    = O(c^{-\beta}(\log c)^{1+\gamma})$;
    linear term~(i) dominant.
\end{itemize}
\end{lemma}

\begin{proof}
Ratios in (ii)--(iii) computed explicitly.
(iv): asymptotic addition
(Proposition~\ref{prop:scale_algebra});
dominance verified per pair
...
(applying~\eqref{eq:Kstar_power} with exponent $1+n\gamma$):
\[
  \normh{A_K^{(n)}}{}_{K=K^*(c)}
...
Apply Lemma~\ref{lem:diagonal_sub} to Theorem~\ref{thm:universal_rate}
at $K=K_{\rm int}=\lfloor K^*(c)\rceil$:
\[
  \normh{\cK-\cKA}{}=O(c^{-\beta}(\log c)^{1+\gamma}).
\]
Leading constant:
$C_{\rm lin}\bigl(\frac{2\beta}{\alpha}\bigr)^{1+\gamma}$ ($c$-admissible).
\end{corollary}
...
\begin{itemize}[nosep]
  \item \textbf{O3a}: does $\exists\,\cAg\in\mathfrak{T}_\gamma$ with
    $\normh{\AKlin}{}=\Omega(K^{1+\gamma}c^{-\beta})$?
    (Open, independent of H1--H2.)
  \item \textbf{O3b}: uniform lower bound would require H3
    (positive-mass / non-cancellation on spectral measure;
    genuinely new axiom, not imposed).
\end{itemize}
...
\begin{itemize}[nosep,leftmargin=1.5em]
  \item $c$-admissible constant chain
    $C_0,C_\infty\to C_{\rm lin},C_{\rm quad}\to C_{\rm tail}$
    (Remark~\ref{rem:cadmissible}, Lemma~\ref{lem:cadm_closure});
  \item three sources of $c$-admissibility: axiomatic (H1/H2),
    analytic (EM), operator-intrinsic ($\BK$ definition);
  \item domain $c$-dependence ($[c_1\log c, c_2\log c]$) strictly
    separated from operator $c$-dependence
    (Remark~\ref{rem:dag_csplit});
  \item fixed-parameter bounds uniform in $K,c$
    via explicit EM remainder~\eqref{eq:EM_remainder};
  \item fixed-$c$ variational $K^*$ on window $[c_1\log c,c_2\log c]$;
  \item slowly-varying substitution stability, uniform in $\gamma$
    on compact subsets
    (eqs.~\eqref{eq:h_rate}--\eqref{eq:Kstar_power});
  \item $\lambda(c)<\tfrac{1}{2}$ is a self-consistency certificate,
    not an existence argument (Lemma~\ref{lem:kopt_variational}(ii'));
  \item single diagonal substitution lemma
    (Lemma~\ref{lem:diagonal_sub}) as unique coupling point;
  \item regime-dependent ordering, verified per pair;
  \item acyclic DAG; Layer-Survival;
    invariants $1+\gamma$, $1+2\gamma$.
\end{itemize}
