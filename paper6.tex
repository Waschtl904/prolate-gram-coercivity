...
\begin{equation}\label{eq:freqdef}
  ...
  \quad\text{(transition-zone IBP frequency, conservative proxy at
  bulk--transition boundary)},
\end{equation}
...
\hline
$\omega_{kl}^{\rm tz}$ & $c^{2/3}|k-l|$ & tz (proxy) & transition
...
enters via one IBP against the bulk oscillation frequency
$\omega_{kl}^{\rm tz}$ \eqref{eq:freqdef} (used as a conservative proxy
at the bulk--transition boundary; see Remark~\ref{rem:freq}).
...
$\omega_{kl}^{\rm tz} \sim c^{2/3}|k-l|$ \eqref{eq:freqdef}
(conservative proxy at the bulk--transition boundary; see Remark~\ref{rem:freq}):
\[
...
\begin{remark}[Origin of the effective frequency $\omega_{kl}^{\rm tz}$]
\label{rem:freq}
The effective frequency $\omega_{kl}^{\rm tz} \sim c^{2/3}|k-l|$ used in the
bulk IBP is a \emph{conservative proxy}: the actual instantaneous WKB frequency
$(\theta_k - \theta_l)'(s) \asymp 2|k-l|/(1-s^2)$ is larger than
$\omega_{kl}^{\rm tz}$ throughout $\mathcal{Z}_{\rm bulk}$
(where $1-s^2 \leq c^{-2/3}$ fails).
Using $\omega_{kl}^{\rm tz}$ as a lower bound for the effective frequency
is conservative and gives a valid (if not optimal) IBP estimate.
The dominant contribution to the integral $I_{kl}^{\rm bulk}$ indeed
comes from $s$ near the matching region $1 - s^2 \sim c^{-2/3}$,
where $(\theta_k - \theta_l)'$ achieves the value $c^{2/3}|k-l|$:
\[
  (\theta_k - \theta_l)'\big|_{1-s^2 = c^{-2/3}}
  \asymp \frac{2c|k-l|}{1-s^2}\cdot\frac{1}{2c\sqrt{1-s^2}}\cdot\sqrt{1-s^2}
...
$\omega_{kl}^{\rm tz} \sim c^{2/3}|k-l|$
(transition-zone IBP frequency \eqref{eq:freqdef}, used as conservative proxy
at the bulk--transition boundary; see Remark~\ref{rem:freq}).
...
\medskip
\noindent\textbf{Sublemma C (Double IBP, SL-based, Fourier-free).}
\emph{Claim:}
\[
...
\]
Both bounds hold independently; each is used separately in the Schur test.
Together they imply $|I_{kl}| \lesssim c^{-2/3}/|k-l|$ (see Conclusion below).

\begin{proof}[Proof of Sublemma C]
Set $f(s) := \arccos(s)$.
We perform two IBP steps in $s$-space using only the SL
equation~\eqref{eq:SLeq}; no Fourier machinery is invoked.

\medskip
\noindent\textbf{Step 1 (First IBP via SL identity).}
From \eqref{eq:SLeq}:
\[
  (\chi_k - \chi_l)\,\phi_{kl}(s)
  = -\frac{d}{ds}W_k^l(s).
\]
...
\[
  ...
  + \text{b.t.}
\]
Boundary terms vanish: $W_k^l|_{s=\pm 1} = 0$ (since $(1-s^2)|_{\pm 1} = 0$);
...
\medskip
\noindent\textbf{Step 2a (Coarse bound -- first claim).}
...
\[
  |I_{kl}^{\rm bulk}| \lesssim \frac{c^{1/2}}{2c|k-l|} = \frac{c^{-1/2}}{2|k-l|},
\]
establishing the first claim.

\medskip
\noindent\textbf{Step 2b (Refined bound via second IBP -- second claim).}
Write $W_k^l(s)/\sqrt{1-s^2} = \sqrt{1-s^2}[\psi_k'\psi_l - \psi_k\psi_l']$.
In $\mathcal{Z}_{\rm bulk}$, WKB gives $\psi_k \approx A_k\cos\theta_k$
with $A_k \asymp c^{-1/4}(1-s^2)^{-1/4}$.
The slowly-varying envelope satisfies:
\[
  g(s) := \sqrt{1-s^2}\,A_k(s)A_l(s)\,\theta_k'(s) \lesssim c^{1/2},
  \qquad \|g\|_{L^1(\mathcal{Z}_{\rm bulk})} \lesssim c^{1/2}.
\]
One IBP against the difference-phase oscillation, using
$\omega_{kl}^{\rm tz} = c^{2/3}|k-l|$ \eqref{eq:freqdef}
as a lower bound for the effective frequency
(conservative proxy at the bulk--transition boundary; Remark~\ref{rem:freq}):
\[
  \int_{\mathcal{Z}_{\rm bulk}} \frac{|W_k^l|}{\sqrt{1-s^2}}\,ds
  \lesssim \frac{\|g\|_{L^1}}{\omega_{kl}^{\rm tz}}
  \leq \frac{c^{1/2}}{c^{2/3}|k-l|}
  = \frac{c^{-1/6}}{|k-l|}.
\]
Substituting into \eqref{eq:step1C}:
\[
  |I_{kl}^{\rm bulk}|
  \leq \frac{1}{2c|k-l|}\cdot\frac{c^{-1/6}}{|k-l|}
  = \frac{c^{-7/6}}{2|k-l|^2},
\]
...
We combine all three sublemmas.
Sublemma C (first claim) gives $|I_{kl}^{\rm bulk}| \lesssim c^{-1/2}/|k-l|$.
For large $c$, $c^{-1/2} \leq c^{-2/3}$ fails (since $1/2 < 2/3$),
so we use the Fourier-cancellation (Sublemma C steps via Proposition~\ref{prop:moment}):
...
\begin{tabular}{lcl}
  ...
  Oscillation frequency (Rem.~\ref{rem:freq}) & $\omega_{kl}^{\rm tz} \sim c^{2/3}|k-l|$ & IBP denominator \\
  ...
  SL double IBP (Sublemma C) & coarse $c^{-1/2}/|k-l|$; refined $c^{-7/6}/|k-l|^2$ & both independent \\
...
(valid for $k,l$ in the transition window, bulk zone \textbf{b}), together with the prefactor $c$:
...
\begin{tabular}{llll}
\hline
\textbf{Mechanism} & \textbf{Factor} & \textbf{Regime} & \textbf{Source} \\
\hline
Commutator: $T_{ii} = 0$ & no diagonal & G & PSWF-orthogonality \\
IBP (bulk oscillation) & $c^{-7/6}/|i-j|$ (Sublm.~B) & b
  & $\omega_{kl}^{\rm tz}\sim c^{2/3}|k-l|$ (Rem.~\ref{rem:freq}) \\
SL double IBP (Sublm.~C) & $c^{-1/2}/|k-l|$ and $c^{-7/6}/|k-l|^2$ & b/G
  & Steps 2a, 2b (independent) \\
Fourier cancellation $\hat\phi_{kl}(0)=0$ & decisive gain & G
  & $L^2$-ortho.~+ Prop.~\ref{prop:moment} \\
Airy Wronskian (integral) & $\int|W_k^l|\lesssim c^{-1/6}/|k-l|$ & mixed
  & Cor.~\ref{cor:Wronskian} \\
Div.\ by $|\chi_k-\chi_l| \asymp 2c|k-l|$ & $\to |\mu_{kl}| \lesssim c^{-1}/|k-l|$ & G
  & Prop.~\ref{prop:moment}(b) \\
$N \sim c^{1/3}$: harmonic sum & $\log N \sim \tfrac{1}{3}\log c$ & G
  & Galerkin cutoff \\
Gram coercivity & well-conditioned $P_N$ & G & Paper~I \\
\hline
\end{tabular}
\caption{Mechanisms contributing to $\|DT_c^{(N)}\| \lesssim c^{-2/3}\log c$
under Assumption~\ref{ass:G}.
\textbf{Regime}: \textbf{G} = uniform in Galerkin window,
\textbf{b} = bulk zone, \textbf{mixed} = combined tz + bulk.
