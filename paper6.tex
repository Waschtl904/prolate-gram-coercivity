...
(i)~$T_{ii}=0$;
(ii)~$|I_{kl}|\leq C_1c^{-1/2}/|k-l|$;
(iii)~the bulk oscillation barrier is localised to
$|k-l|\lesssim c^{1/6}$;
(iv)~$\|T\|\geq\tfrac{\kappa}{2}\log c$ is optimal in the
pointwise-bound class (No-go Theorem).
These are the primary analytic contributions.

\smallskip\noindent\textbf{Layer~1 (Assumption~B-strong):}
$\norm{D\mathcal{T}_c^{(N)}}\leq C(A)\log c$.
Assumption~B splits into B-weak (mechanically derived from
transition asymptotics, $P_{kl}\lesssim c^{5/3}|k-l|$) and
B-strong ($P_{kl}\leq C_2c^{1/2}$, requires phase-sensitive
...
pointwise-bound class; contraction requires oscillatory
cancellation.
...
No pointwise argument establishes $\norm{DT}<1$.
\end{theorem}
...
\begin{enumerate}[nosep,label=(\arabic*)]
  ...
  \item $|I_{kl}|\leq C_1c^{-1/2}/|k-l|$ (Lemma~\ref{lem:RL});
  \item barrier localised to $|k-l|\lesssim c^{1/6}$
        (Proposition~\ref{prop:bulk_barrier});
  \item $\|T\|\geq\tfrac{\kappa}{2}\log c$ optimal
        (Theorem~\ref{thm:nogo}).
\end{enumerate}
Items~(1)--(3) are the primary analytic content.
Item~(4) eliminates an entire class of approaches.
...
\medskip\noindent
The paper's contribution is Layer~0: unconditional geometry
of the obstruction.
Layers~1--2 identify what remains and why it cannot be
obtained by further pointwise analysis.
...
\begin{enumerate}[nosep,label=(\arabic*)]
  ...
  \item \textbf{Schur optimality} (Thm~\ref{thm:nogo}):
        $\log c$ sharp in $\{|T_{ij}|\leq C/|i-j|\}$.
...
Layer~1 bound is optimal; Layer~2 requires oscillatory cancellation.}
\label{tab:scales}
\end{table}
...
Airy asymptotics.
This is the precise content of ``phase-sensitive Airy--WKB
matching'' in Open Problem~\ref{op:prefactor}.
\end{remark}
...
The apparent circularity dissolves because both Conjectures are
statements about \emph{PSWF asymptotics at the single point $s_*$},
computable in principle from Airy--WKB connection formulae
independently of the contraction problem.
They are strictly weaker than the contraction conclusion.
\end{remark}
...
\begin{proof}
$P_{ij}\cdot|I_{ij}|\leq C_2c^{1/2}\cdot C_1c^{-1/2}/|i-j|=C_1C_2/|i-j|$.
\end{proof}
...
$|I_{ij}|\leq C_1c^{-1/2}/|i-j|$&---&Lem.~\ref{lem:RL}&0\\
Barrier$\subseteq\{|k-l|\lesssim c^{1/6}\}$&---
  &Prop.~\ref{prop:bulk_barrier}&0\\
$\norm{T}\geq\tfrac{\kappa}{2}\log c$ optimal&---&Thm~\ref{thm:nogo}&0\\
\hline
$P_{kl}\leq C_2c^{1/2}$&B-strong&Ass.~\ref{ass:Bstrong}&1 (open)\\
$|T_{ij}|\leq C_1C_2/|i-j|$&---&Prop.~\ref{prop:Tij}&1\\
$\norm{T}\leq C\log c$&optimal&Thm~\ref{thm:main}(a)&1\\
\hline
Conj.~\ref{conj:phase} + Conj.~\ref{conj:amplitude}&---&---&2 (open)\\
$\norm{T}\leq Cc^{-1/2}\log c\to0$&---&Thm~\ref{thm:main}(b)&2\\
\hline
\end{tabular}
\caption{Proof architecture. Layer~0 unconditional;
Layer~1 conditional on B-strong;
Layer~2 conditional on both Conjectures.}
\label{tab:mechanisms}
\end{table}

\begin{corollary}[Conditional fixed point]\label{cor:fixedpoint}
Under B-strong and B$'$: unique $g^{(N)}$ with
$\mathcal{T}_c^{(N)}(g^{(N)})=g^{(N)}$;
Picard rate $\leq Cc^{-1/2}\log c$.
\end{corollary}
...
By Theorem~\ref{thm:nogo}: no pointwise improvement achieves this.
...
\medskip\noindent\textbf{Important caveat (two open points).}
\begin{enumerate}[nosep,label=(\arabic*)]
...
\end{enumerate}
Conditional on both Conjectures, the question
$\norm{g}_{L^\infty(\mathbb{T})}<1$ is finite and computable;
the Szeg\H{o}--Toeplitz theorem then gives
...
and control of the correlation between $|\lam{k}-\lam{l}|$ and
$(1-\lam{k})(1-\lam{l})$ as joint functions of $k$ via Airy.
\end{openproblem}
  ...
  \item Control $\norm{T^{\rm near}}$:
        \begin{enumerate}[nosep,label=(\alph*)]
          ...
          \item Homogenisation: prove
                ...
                (requires Conj.~\ref{conj:amplitude}; separate OP).
        \end{enumerate}
\end{enumerate}
\end{openproblem}
...
This Lipschitz-in-$j$ condition on the normalised amplitude
should follow from PSWF transition asymptotics but is not
established.
...
\subsection*{Layer~0: unconditional geometry (primary content)}
$T_{ii}=0$;\;
...
$\norm{T}\leq C\log c$.
B-strong requires phase-sensitive Airy coupling (OP~\ref{op:prefactor});
B-weak is insufficient (Rem.~\ref{rem:balance}).

\subsection*{Layer~2: conditional on B-strong $+$ B$'$}
$\norm{T}\leq Cc^{-1/2}\log c\to0$;\;
conditional fixed point (Cor.~\ref{cor:fixedpoint}).
B$'$ requires Conj.~\ref{conj:phase} (phase) \emph{and}
Conj.~\ref{conj:amplitude} (amplitude); neither alone suffices.

\subsection*{What is genuinely new}
All three items are Layer~0 and independent of B, B$'$:
\textbf{(1)} barrier localisation (relative width $c^{-1/6}\to0$);
\textbf{(2)} $\log c$ Schur optimality;
\textbf{(3)} near/far band decomposition.

\subsection*{Core diagnosis}
\textit{Local WKB asymptotics do not determine global operator
cancellation.}
Layer~0 provides the complete local geometry of the obstruction.
The transition from local to global is the true open problem,
separated into OPs~\ref{op:prefactor}--\ref{op:amplitude}.
...
(scaling model vs.\ limit operator, two open points),
the hardness remark on weighted norms, and the core diagnosis
``local WKB asymptotics do not determine global cancellation''
