...
\medskip\noindent
\textbf{Epistemic coupling with B$'$.}
A structural question complementary to the above concerns the
logical independence of B-strong from Assumption~B$'$:
any proof of $P_{kl}\leq C_2c^{1/2}$ will necessarily use
the Airy-phase data at $s_*$, the same data that drives the
oscillatory cancellation in B$'$.
This does not create a logical dependency: $T_{ij}=P_{ij}\cdot I_{ij}$
is an exact product and the implication chain
B-strong $+$ B$'$ $\Rightarrow$ Layer~2 is unaffected.
However, it creates an \emph{epistemic coupling}: the two
assumptions share a common asymptotic origin at $s_*$, and
a proof of B-strong may well import oscillatory structure
already implicit in B$'$.
If so, B-strong would not be a genuinely separable hypothesis,
but rather an intermediate consequence of the full
phase-amplitude structure required by B$'$.
This does not affect the validity of any result stated here,
but it is noted explicitly for the programme: the
conceptually cleanest path may be to prove B$'$ directly
(via Conjectures~\ref{conj:phase}--\ref{conj:amplitude})
rather than factoring through B-strong as an intermediate step.
\end{remark}
...
See also the epistemic coupling remark in~\ref{rem:Bstrong_content}:
it may be more efficient to prove B$'$ directly
via Conjectures~\ref{conj:phase}--\ref{conj:amplitude}
than to first establish B-strong as an intermediate step.
\end{openproblem}
...
B-strong shares asymptotic origin at $s_*$ with B$'$;
its epistemic status as a separable hypothesis is open
