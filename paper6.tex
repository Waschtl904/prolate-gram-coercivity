...
but the unique crossover exponent forced by the simultaneous
interaction of three independent asymptotic regimes.

\medskip\noindent
\textbf{(i) Galerkin spectral scaling.}
The natural dimension of the PSWF transition regime forces
the index truncation $N\sim c^{1/3}$, so all admissible
index separations satisfy $|k-l|\leq N\sim c^{1/3}$.

\medskip\noindent
\textbf{(ii) Airy transition width.}
The turning point $s_*=1-c^{-2/3}$ is the boundary of the
Airy regime; the spatial resolution scale of the transition
zone is $\Delta s\sim c^{-2/3}$.

\medskip\noindent
\textbf{(iii) Second-IBP boundary amplification.}
In the bulk, the difference-phase oscillation frequency is
$\partial_s(\theta_k-\theta_l)\asymp|k-l|(1-s^2)^{-1/2}$,
so the second integration-by-parts produces a boundary term
at $s_*$ of size
\[
  \text{boundary term} \;\asymp\;
  \frac{c^{-1/2}}{|k-l|} \cdot \frac{1}{\Delta s \cdot |k-l|(1-s_*^2)^{-1/2}}
  \;=\;
  \frac{c^{-1/2} \cdot c^{2/3}}{|k-l|^2 \cdot c^{-1/3}}
  \;=\;
  \frac{c^{1/2}}{|k-l|^2}.
\]
The second IBP succeeds (boundary term $\ll$ target $c^{-1/2}/|k-l|$)
precisely when
\[
  \frac{c^{1/2}}{|k-l|^2} \ll \frac{c^{-1/2}}{|k-l|}
  \quad\Longleftrightarrow\quad
  |k-l| \gg c^{1/2} \cdot c^{-1/2} \cdot c^{1/6}
  \;=\; c^{1/6}.
\]

\medskip\noindent
\textbf{Balance equation.}
Combining: the obstruction is active for
\[
  |k-l| \;\lesssim\; c^{1/6}
  \;=\; c^{1/3} \cdot c^{-2/3} \cdot c^{1/2}
  \;=\; N \cdot \Delta s \cdot c^{1/2},
\]
where the three factors correspond respectively to
(i) Galerkin index range, (ii) Airy transition width,
and (iii) IBP amplitude scale.
The exponent $1/6 = 1/3 - 2/3 + 1/2$ is the unique value
compatible with all three regimes simultaneously;
no alternative power $c^\alpha$ with $\alpha\neq 1/6$
satisfies the balance condition for all $c\gg1$.

\medskip\noindent
In particular, $c^{1/6}$ is \emph{not} a design choice
or an artifact of the proof method:
it is the unavoidable crossover scale at which the second
integration-by-parts transitions from effective to obstructed.
\end{remark}
...
Conjectures~\ref{conj:phase} and~\ref{conj:amplitude} are
logically independent (one concerns phase arithmetic,
the other amplitude regularity) but jointly necessary for B$'$.
\end{openproblem}
...
The $c^{1/6}$ interaction scale is not a free parameter but the
unique crossover exponent forced by Galerkin spectral scaling,
Airy transition width, and second-IBP boundary amplification
...
\textbf{(4)} mechanical derivation of the $c^{1/6}$ crossover
scale from three independent asymptotic regimes
(Remark~\ref{rem:c16_scale}).
...
the mechanical derivation of the $c^{1/6}$ crossover scale
(Remark~\ref{rem:c16_scale}),
and the core diagnosis ``local WKB asymptotics do not determine
global cancellation'' were developed in working sessions, May 2026.
