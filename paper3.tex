...
All terms are well-defined in $L^2([-T,T])$ since
$f_{mn} \in \mathcal{D}(q_{D_c})$ (Remark~\ref{rem:form-domain}).
\end{proof}

\begin{remark}[Coefficient-level consequence]
\label{rem:near-eig-consequence}
For $\chi_k \ne \mumN$:
$|a_k^{mn}| \le \|R_{mn}\|_{L^2}/|\chi_k-\mumN|$.
\end{remark}
...
\begin{equation}\label{eq:chi-gap}
  \chi_k(c) - \mumN \ge c_2(\gamma)\,N^2,
  \qquad c_2(\gamma) = (1-2\gamma^2)/2 > 0.
\end{equation}
\end{lemma}

\begin{lemma}[Gap: $\mumN$ vs.\ tail eigenvalues]\label{lem:gap-od}
...
= \ip{D_c f_{mn}}{f_{mn}}
= \ip{\mumN f_{mn}+R_{mn}}{f_{mn}}
= \mumN\|f_{mn}\|^2 + \ip{R_{mn}}{f_{mn}}$.
Divide by $\|f_{mn}\|^2$.
\end{proof}
...
The analogy with Clebsch--Gordan is heuristic: PSWFs carry no
Lie-algebraic structure, and the decomposition is not into
...
\begin{enumerate}[(i)]
  ...
  in the interior.
  \item \emph{Mean eigenvalue shift:} $\mumN/2$ --- the reference
  energy level.
\end{enumerate}
...
\begin{equation}\label{eq:spectral-lower}
  \Emn[\chi_k] \ge \frac{\mumN}{2}.
\end{equation}
\end{proposition}

\begin{proof}
From~\eqref{eq:energy-intro}:
$\Emn[\chi_k] = \mumN/2 + A + B$
where
...
eigenvalues much smaller than $\mumN/2$.
Combined with Proposition~\ref{prop:mean-loc}, which shows
$\Emn[\chi_k] \approx \mumN$, the two bounds together confine
the spectral mean to the interval $[\mumN/2, \mumN + O(N^2)]$.
The gap between $\mumN/2$ and $\mumN$ is the remaining
...
The near-eigenvector target $\mumN \sim 2N^2+c^2 > \chi_N$,
so the concentration set $\{k:\chi_k \approx \mumN\}$
...
At the edge, $\mumN > \chi_N$ and $\Eout = O(1)$ simultaneously.
...
%% [PATCH D] Semiclassical turning-point interpretation
Geometrically, the edge regime corresponds to a failure of
the semiclassical approximation at the turning points
$x = \pm T$: when $m, n \sim N$, both classical orbits
reach the boundary of the interval, spectral crowding prevents
separation of scales between adjacent eigenvalues, and
the WKB phase ceases to be monotone near $|x| = T$.
This is the microlocal mechanism underlying both the
Slepian obstruction and the energy concentration.
\end{remark}
...
\emph{Status: resolved by Paper~IV \cite{PaperIV}.}
Paper~IV, Theorem~1, proves hypothesis~\eqref{eq:weak-conv-hyp}
of Lemma~\ref{lem:bulk-reduction}:
for every $f \in C^1([-T,T])$ and $n \le \gamma N$,
\begin{equation}\label{eq:pswf-weak-lim}
  \int_{-T}^T f(x)\,\psi_n(x)^2\,dx
  = \lambda_n(c)\int_{-T}^T f(x)\,\rhocl(x)\,dx
    + O\!\left(\frac{\lambda_n(c)\|f\|_{C^1}}{n}\right),
\end{equation}
where $\rhocl$ is the classical equilibrium density
(Definition~\ref{def:classical}).
The proof uses Pr\"{u}fer phase monotonicity
(Paper~IV, Lemma~2) and a 1D Riemann--Lebesgue argument;
it does not invoke WKB approximation.
Two extensions remain open:
\begin{enumerate}[(P1)]
  \item Extension of~\eqref{eq:pswf-weak-lim} to $f \in C^0$
  (non-critical for the bulk program).
  \item Off-diagonal analogue: weak convergence of
  $\psi_m\psi_n$ for $m \ne n$ (future work, needed for
  Problem~\ref{prob:cg}).
\end{enumerate}
\end{problem}
...
Prove that the spectral mean is asymptotically close to $\mumN$:
\begin{equation}\label{eq:comm-refined}
  \frac{\Emn[\chi_k]}{\mumN} \to 1
  \qquad \text{as } N \to \infty,\; m,n \le \gamma N.
\end{equation}
By Lemma~\ref{lem:bulk-reduction} and Paper~IV \cite{PaperIV},
this is now unconditionally resolved for $m,n \le \gamma N$
(given Assumption~\ref{ass:lw}).
...
constraint; the challenge was to transfer it to the
...
\bibitem{PaperIV}
Anonymous Author,
\emph{Semiclassical Equidistribution of PSWF Densities
and Closure of the Bulk Decorrelation Program},
preprint, 2026.
