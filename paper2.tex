...
In the current Reinschrift of Paper~I, condition~(T.2) is verified
...
version of~\textup{(T.2)} along the sequence.
...
\item \emph{explicitly stable and Weil-decomposable} under the abstract
...
condition~(T.2) for prime sampling; that issue is left open there.
...
$\lambda_{\min}(\GVMk)$ requires a comparison estimate of type~(T.2) along the
prime sequence. Since this is open in Paper~I, we isolate it here as an explicit
assumption rather than importing it as a theorem.
\end{enumerate}

\subsection{What this paper does not claim}

No statement is made about zeros of $\zeta(s)$.
We do not prove that prime sampling satisfies condition~(T.2).
...
\item Under condition~\textup{(T.2)} of \cite{PaperI}, one has the spectral stability bound
$\lambda_{\min}(\GVM)\geq\lambda_{N-1}(c)-\varepsilon_N$.
\item Under condition~\textup{(T.2)} of \cite{PaperI}, one has the comparison formula
$Q^{(N)}(f)=\ip{\Kop f}{f}_{L^2}+R_N(f)$ with
$|R_N(f)|\leq \varepsilon_N\norm{f}_{L^2}^2$.
\end{enumerate}
Moreover, in the Reinschrift of Paper~I, condition~\textup{(T.2)} is established for
...
satisfy condition~\textup{(T.2)} of \cite{PaperI} with parameter $\varepsilon_{N_k}$
for every $k$.
...
In the Reinschrift of Paper~I, prime sampling is not proved to satisfy~(T.2);
...
For part (II), Assumption~\textup{(H)} provides condition~(T.2) along the sequence,
so Theorem~\ref{thm:paperI}(III) applies at each level $k$ and yields
\[
  \lambda_{\min}(\GVMk)
  \ge \lambda_{N_k-1}(c_k)-\varepsilon_{N_k}.
\]
...
$\GVMk=G^{\mathrm{ref}}_{N_k,c_k}+E_{N_k}$ under condition~(T.2), taking traces,
and estimating
...
\begin{problem}[Prime sampling and condition~\textup{(T.2)}]\label{prob:primeT2}
Prove or disprove Assumption~\textup{(H)} for prime sampling.
More precisely, determine whether the prime sets
$\mathbf p_k=(2,3,5,\ldots,p_{N_k})$ satisfy the comparison estimate of Paper~I
with an error $\varepsilon_{N_k}\to0$ in the regime $N_k/c_k\to\rho\in(0,2/\pi)$.
