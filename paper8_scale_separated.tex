...
%  Status: working draft (revised v4: submission-ready light)
% ============================================================
...
\emph{Rigorous.}
Under Assumption~\ref{ass:gap_only} (uniform spectral gap
...
This cancellation depends only on monotonicity and a
uniform gap lower bound; no Airy or erfc structure
is required.
...
to Assumption~\ref{ass:gap_only}.
This paper is a \emph{conditional reduction theorem}:
under Assumption~\ref{ass:gap_only} and B-strong,
...
\item \emph{Combinatorial scale} (Dyadic Separation):
  ...
  $(c/2)^{-1/3}$; this is Assumption~\ref{ass:gap_only},
...
\item \emph{Local scale} (Airy / WKB):
  ...
  Assumption~\ref{ass:gap_only} and the upper bounds
...
Assumption~\ref{ass:gap_only}, Proposition~\ref{prop:U},
...
\begin{assumption}[Minimal Spectral Input]
\label{ass:gap_only}
There exist $\kappa_0 > 0$ such that for all
...
\begin{remark}[Routes to Assumption~\ref{ass:gap_only}]
\label{rem:gap_route}
...
Assumption~\ref{ass:gap_only} with
$\kappa_0 = \tfrac{1}{2}c_{\min}$.
\end{proposition}
...
\label{rem:scales}
Assumption~\ref{ass:gap_only} and Lemma~\ref{lem:F}
...
\label{lem:F}
Under Assumption~\ref{ass:gap_only},
...
\label{rem:Frobust}
The proof uses only Assumption~\ref{ass:gap_only}:
...
\item Assumption~\ref{ass:gap_only} (gap lower bound);
\item An upper bound
...
\smallskip
\emph{The upper bound in~\textup{(b)} is implied by
Proposition~\ref{prop:U}\textup{(U')};
see~\eqref{eq:incr_simple} of Lemma~\ref{lem:A}.}
\end{corollary}
...
\label{lem:E}
Fix $m_0 \ge 1$; assume Assumption~\ref{ass:gap_only},
...
\label{rem:deps}
\begin{align*}
  &\text{Ass.~\ref{ass:gap_only}}
    \Rightarrow\text{Lem.~\ref{lem:F}}
    \Rightarrow\text{Cor.~\ref{cor:C} (Term 1)};\\
  &\text{Prop.~\ref{prop:U}(U')}
    \Rightarrow\text{Lem.~\ref{lem:A}}
    \Rightarrow\text{Cor.~\ref{cor:C}\,(b) + Term 2};\\
  &\text{Prop.~\ref{prop:U}(U)}
    \Rightarrow\text{Ass.~\ref{ass:gap_only} (one route)};\\
  &\text{Cor.~\ref{cor:C} + H3}
    \Rightarrow\text{Lem.~\ref{lem:E}}.
\end{align*}
\textbf{Key point:}
Lemma~\ref{lem:F} requires only
Assumption~\ref{ass:gap_only}.
Proposition~\ref{prop:U} is one sufficient condition
for both (a) and (b) of Corollary~\ref{cor:C},
...
\label{rem:open}
\begin{itemize}
\item \textbf{Assumption~\ref{ass:gap_only}:}
