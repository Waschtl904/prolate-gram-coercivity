...
\item \emph{Local scale} (Airy / WKB): provides a
  ...
  and the upper bounds needed for Term~2 of
  Corollary~\ref{cor:C}, but is not the logical
  foundation of the Dyadic Separation Principle.
\item \emph{Global scale} (Landau--Widom): the eigenvalue
  profile follows an error-function law with
...
Proposition~\ref{prop:gap}, Proposition~\ref{prop:U},
...
\begin{remark}[Role and routes to Proposition~\ref{prop:gap}]
\label{rem:gap_route}
...
Three routes to~\eqref{eq:gapbound} are plausible:
\begin{enumerate}[(i)]
\item Via Proposition~\ref{prop:U}(U): for $c$
  sufficiently large, the Airy approximation implies
  $\kappa_0 = \tfrac{1}{2}c_{\min}$.
\item Via first-order WKB: a monotone comparison
  argument at leading order, without Airy matching.
\item Via numerical lower bounds: direct computation
  for finite $c$ combined with asymptotic stability.
\end{enumerate}
...
such that for all sufficiently large $c$ and all $l$ with
$|l-\NSh| \le A_0 c^{1/3}$:
\begin{itemize}
...
at the combinatorial scale, requiring only gaps bounded
below at order $c^{-1/3}$.
...
Proposition~\ref{prop:gap}: strict monotonicity plus
a local lower gap bound at scale $(c/2)^{-1/3}$.
It requires neither the Airy form of
$\lambda_l - \lambda_{l+1}$ nor the erfc form of
$\lambda_l$.
...
\label{cor:C}
Fix $m_0\ge 1$.
Under Proposition~\ref{prop:gap} and
Proposition~\ref{prop:U}\textup{(U')}, define
\[
  \delta_m
...
\label{lem:E}
Fix $m_0 \ge 1$; assume Proposition~\ref{prop:gap},
Proposition~\ref{prop:U}, and B-strong \cite{paperVI}
($A_{ij} \le C_2$).
...
\label{cor:closure}
Under Proposition~\ref{prop:gap}, Proposition~\ref{prop:U},
and B-strong,
...
for Term~2 in Corollary~\ref{cor:C} and one route
to Proposition~\ref{prop:gap}, but it is not the
logical foundation of the Dyadic Separation Principle.
All constants depend only on $A_0, C_1, C_2, c_{\min}$.
\end{remark}
...
\item \textbf{B-strong:} $P_{kl}\le C_2 c^{1/2}$
  in the transition zone; no proof available.
