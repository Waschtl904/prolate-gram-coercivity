...
% ---- FIX 1: DSTP necessity remark --------------------------------
\begin{remark}[Necessity of DSTP]
\label{rem:dstp_necessity}
DSTP is not merely sufficient for the main theorem; it is
\emph{necessary} in the following sense: any quadrature rule for
which $\|E_{N,c}\|_2 = o(hc \cdot N)$ must satisfy a condition of
DSTP type, since the structure-blind bound $|E_{mn}|\le Chc$
(Theorem~\ref{thm:banded_decay}) is sharp for generic rules
(Remark~\ref{rem:ax5_independence}). DSTP is therefore the minimal
additional condition that separates stability from instability at
operator level. It is a non-uniform spectral localisation condition
in the PSWF eigenbasis, not a sampling density condition.
\end{remark}
% ------------------------------------------------------------------
...
% ---- FIX 3: gamma_0 scaling clarification ------------------------
...
% ---- FIX 2: explicit kappa_1 uniformisation ----------------------
\smallskip
...
of $c$ and of which regime applies. The IBP argument
(Lemma~\ref{lem:ibp_uniform}) depends only on positivity of $\kappa$,
not on its precise value.
\end{proof}
...
\[
  ...
  \;\ge\; c_0\,(hc)^2 N,
\]
with $c_0 > 0$ independent of $c \ge c_0$. Therefore:
\[
