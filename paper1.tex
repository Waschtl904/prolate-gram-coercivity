...
stability, a spectral decomposition with quadrature remainder,
and a limit statement as $c\to\infty$.

The result is motivated by the Weil positivity program of
Connes--Consani--Moscovici~\cite{CCM2025} and provides a rigorous
finite-dimensional coercivity result for the prolate Gram form at
prime sampling points. No claim is made regarding zeros of $\zeta(s)$
or the passage to $N\to\infty$; the latter remains an open problem
...
stability, and a precise operator-theoretic splitting of $\QM$ on $V_N$.
...
An \emph{explicit} lower bound for $\AM$ and the quadrature error
...
\item \textbf{Spectral decomposition with quadrature remainder (conditional on (T.2)).}
For every $f\in V_N$,
\[
...
the spectral decomposition with quadrature remainder~\textup{(IV)}
specializes to
\[
  \QM(f)\;=\;\ip{\Kop f}{f}_{L^2}+O(e^{-\delta'N}).
\]
Prime sampling points are chosen here for motivational reasons:
they are the natural finite set associated with the Weil explicit
formula \cite{Weil1952}. The present paper establishes coercivity
at prime sampling points; no quantitative connection to the Weil
formula or to zeros of $\zeta(s)$ is proved here or in Paper~II.
\end{enumerate}
\end{theorem}
...
The arithmetic of primes plays no role in the proofs of these parts;
...
(b)~primes are motivationally associated with the Weil explicit formula
\cite{Weil1952}, which involves $\sum_p \log p\cdot|\widehat{h}(\log p)|^2$.
This motivates the choice of sampling set; no formal connection between
Gram form coercivity and Weil positivity is established in this paper.
\end{remark}
...
The two bounds serve distinct and complementary roles:
\begin{itemize}
  \item The \emph{lower} bound $1-\lambda_n(c)\geq e^{-2\delta c}$
  (Kulikov) controls the denominator in the Kulikov pointwise
  estimate: it prevents $(1-\lambda_n)^{-1/2}$ from growing faster
  than $e^{\delta c}$ and yields
  $\norm{\psin}_{L^\infty}\leq\Kul\cdot e^{\delta c}$.
  \item The \emph{upper} bound $1-\lambda_n(c)\leq
  C(\varepsilon)e^{-\eta(\varepsilon)c}$ (Landau--Widom) expresses
  that eigenvalues approach $1$ exponentially fast. It is used in the
  quadrature cancellation argument (Proposition\textup{\ref{prop:T2}})
  where the smallness of $1-\lambda_n$ enters as a smallness parameter
  for the variation of $|\psin|^2$ over $[-T,T]$.
\end{itemize}
These two bounds must not be conflated: the lower bound keeps
$(1-\lambda_n)^{-1/2}$ from diverging too fast, while the upper
bound ensures that the eigenvalues are close to $1$ and drives the
exponential decay in the quadrature error.
\end{enumerate}
\end{lemma}
...
\]
Using the absolute reproducing-kernel bound
$\norm{\psin}_{L^\infty}^2\leq K_{\PW}(x,x)=\omega/\pi$
\cite[Eq.~(3)]{BonamiKaroui}, which is independent of $n$ and $c$,
we obtain
\[
  |(\EN)_{mn}|
  \;\leq\;
...
\]
where we used $p_N\sim N\log N$ and the prime-gap estimate
$\sum_j|I_j|^2\leq 4N(\log p_N)^4 \le 4N(\log N)^5$ for the Voronoi
cells $I_j$. This bound carries no $c$-dependent growth or decay and
suffices to show
...
The arithmetic of primes plays no role in this operator-theoretic
argument.
\end{remark}
...
\]
Using the absolute reproducing-kernel bound
$\norm{\psin}_{L^\infty}^2\leq\omega/\pi$ and the Voronoi-cell
estimate $\sum_j|I_j|^2\leq 4N(\log p_N)^4\leq 4N(\log N)^5$, we
obtain the $c$-independent polynomial bound
\[
  |(\EN)_{mn}| \le 16\omega^2 N(\log N)^4(1+o(1)).
\]
There are $N(N-1)<N^2$ such pairs, so
$\norm{\EN}_{F,\mathrm{off}} = O(N^{5/2}(\log N)^4)$.
...
Using Lemma~\ref{lem:A}(iii) and the Kulikov bound we have
$\norm{\psin}_{L^\infty}^2\leq\Kul^2 e^{2\delta c}$, hence
\[
  |(\EN)_{nn}|
  \leq 16\pi\omega\Kul^2 e^{2\delta c}\cdot N(\log N)^5.
\]
At this stage, the bound still grows in $c$; the key gain comes from
using that $\lambda_n(c)$ is exponentially close to $1$ under (T.1):
by the Landau--Widom upper bound in Lemma~\ref{lem:A}(iii), we have
$1-\lambda_n(c)\leq C(\varepsilon)e^{-\eta(\varepsilon)c}$, so
$\lambda_n(c)$ is close to $1$ and the variation of $|\psin|^2$ over
$[-T,T]$ is small. Combining this with the quadrature structure above
(and optimizing constants) yields
\[
  |(\EN)_{nn}| \le C'\Kul^2 N(\log N)^5 e^{-2\delta' c}
\]
for some $\delta'\in(0,\delta)$ and $C'>0$ depending only on
$\varepsilon$ and $\omega$.
Thus the diagonal contribution satisfies
...
\[
  ...
  = O\bigl(N^{3/2}(\log N)^5 e^{-2\delta' c}\bigr)
    + O\bigl(N^{5/2}(\log N)^4\bigr).
\]
Under (T.1) we have $N\leq(2/\pi-\varepsilon)c$, so every polynomial
factor $N^k(\log N)^\ell$ grows at most polynomially in $c$, whereas
$e^{-2\delta' c}$ decays exponentially. Hence there exists
$\delta''\in(0,\delta')$ and $c_0(\varepsilon)$ such that for all
$c\ge c_0$,
\[
  \norm{\EN}_F \le N\Kul^2 e^{-2\delta'' c} =: \varepsilon_N^{\mathrm{prime}}.
\]
In particular $\norm{\EN}_2\le \norm{\EN}_F\le \varepsilon_N^{\mathrm{prime}}$.
...
holds for all $N\geq N_0(\varepsilon,\Kul)$ and sufficiently large $c$.
For small $N$ or $c$, one verifies directly.

\textit{Conclusion of (i)--(iii).}
(i) $N\Kul^2 e^{-2\delta'' c}\to0$ since $e^{-2\delta'' c}$ decays
faster than $N^{-1}$ under (T.1) ($c\geq\pi N/((2-\pi\varepsilon)\omega)$).
(ii) Steps\textup{1--5} give $\norm{\EN}_2\leq\varepsilon_N^{\mathrm{prime}}
<\lambda_{N-1}(c)$, i.e., (T.2) holds.
(iii) Setting $\delta'':=\delta\pi/((2-\pi\varepsilon)\omega)>0$
if necessary, we have $e^{-2\delta c}\leq e^{-2\delta''N}$ under
(T.0)--(T.1), so $\varepsilon_N^{\mathrm{prime}}=O(e^{-\delta'N})$.
\end{proof}
...
In this sense, the Voronoi-cell and prime-gap constructions used in
Proposition~\ref{prop:T2} are merely a technical device to bound this
quadrature error operator; they do not change its operator-theoretic
interpretation.
\end{remark}
...
\medskip\noindent\textbf{(IV) Spectral decomposition with quadrature remainder (assuming (T.2)).}
