...
DSTP is logically independent of the remaining axioms
\textup{AX1--AX4} (symbol class, spectral asymptotics, quadrature
consistency, uniform PSWF bounds). The independence is witnessed
...
This rule satisfies Assumption~\ref{ass:quad_first_order} (with
$C_Q = 1$, $h = 2T/N$) and the uniform PSWF bounds of
\cite{BonamiKaroui2014}, since both conditions are intrinsic to the
PSWFs and independent of the node locations.
...
phase-alignment with the oscillatory structure of
$\psi_m^{(c)}\overline{\psi_n^{(c)}}$, so $R_{mn}^{\mathrm{quad}}$
cannot exploit destructive interference.
Consequently, $\|E_{N,c}\|_2 \lesssim hc \cdot N$
(Schur test with no off-diagonal decay), and for $N \sim c$ this
diverges, yielding no frame stability from
Theorem~\ref{thm:conditional_frame_schur}.

\smallskip
\noindent\textbf{Structural interpretation.}
The failure of DSTP for the Riemann rule is not a consequence of low
approximation order or insufficient regularity. The symbol-class
control (AX1), spectral gap (AX2), quadrature consistency (AX3), and
uniform PSWF bounds (AX4) together supply no information about
\emph{geometric compatibility} between the node set $\{p_j\}$ and the
oscillatory eigenstructure of $V_{N,c}$. DSTP is not an estimate of
higher order; it is a condition of a different type entirely:

\begin{quotation}
  \noindent\textit{DSTP is not an estimate. It is a geometric
...
non-uniform sampling theory --- all of which assert structural
resonance between a discrete node set and a continuous function
class, and none of which follow from regularity bounds alone.

\smallskip
\noindent\textbf{Minimality.}
DSTP is not ``stronger'' than AX1--AX4; it is \emph{orthogonal} to
them: AX1--AX4 control the \emph{analytic} structure of the system
(regularity, spectral gaps, approximation order); DSTP controls the
\emph{geometric} compatibility of the discretisation. No combination
of analytic bounds implies geometric compatibility. The axiom system
is therefore minimal: no axiom is derivable from the others.
