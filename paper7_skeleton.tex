...
where
\[
  \sup_{m\leq \lfloor \log_2 N\rfloor} \epsilon_m^{(N)} \to 0
...
borderline non-summability $\sum_n n^{-1} = \infty$,
...
rescaled kernel $P_{ij}\,c^{1/2}$. The bound $O(1)$ in
Theorem~\ref{thm:dyadic_cancellation} therefore corresponds
to $O(c^{-1/2})$ for the original operator. The additional
$\log c$ factor arises from the number of dyadic blocks
$M \sim \log N \sim \tfrac{1}{3}\log c$.
\end{remark}
...
\[
  ...
  = \frac{1}{2^m}
    \sum_{j\in\Bm(i)}
    A_{ij}\,e^{i\alpha(i-j)}(1+O(\epsilon_m^{(N)})).
\]
...
$\mathrm{TV}(a;\Bm)=O(\delta_m)$ uniformly in $i$.
...
error $O(\epsilon_m^{(N)})$ yields
\[
  S_m(i)=O\!\left(\frac{1}{2^m}\right)
  + O\!\left(\frac{\delta_m}{2^m}\right)
  + O\!\left(\frac{\epsilon_m^{(N)}}{2^m}\right),
\]
uniformly in $i$.
...
\[
  \sup_{1\leq i\leq N}\sum_{j\neq i}\abs{T_{ij}}
  \leq \sum_{m=0}^M \sup_i \abs{S_m(i)}
  = O\!\left(\sum_{m=0}^M \frac{1}{2^m}\right)
  = O(1),
\]
since $\delta_m\to 0$ and
$\sup_{m\leq \log N}\epsilon_m^{(N)}\to 0$.
\end{proof}
...
\begin{tikzpicture}[
  ...
  \node[box, below=of Norm]  (Small) {$\|D\mathcal{T}\|<1$};
