...
%% To be inserted into paper2_quadrature.tex before \end{document}
%%============================================================
...
of PSWFs (\cite{OsipovRokhlinXiao}, Ch.~3)
and a high-resolution Gauss--Legendre quadrature
for all reference inner products.
All experiments were performed in double precision (64-bit floating point).
...
(see \cite{OsipovRokhlinXiao}, Ch.~3):
\begin{align*}
  T^{(c)}_{kk} &= k(k+1)
...
\end{align*}
Eigenvectors are sorted in order of increasing prolate eigenvalue,
yielding Legendre coefficients
...
\[
  \max_{\,i,j \le N_{\mathrm{test}}}|\langle\psi_i,\psi_j\rangle - \delta_{ij}|
  < 10^{-12},
\]
with $N_{\mathrm{test}} = 20$ for $c=50$ and $N_{\mathrm{test}} = 30$ for $c=100$.

The spectral projection $\PN$ is defined by
\[
  \PN f := \sum_{k=0}^{N-1} \langle f, \psi_k^{(c)} \rangle_{L^2}
  \psi_k^{(c)},
\]
with inner products evaluated via Gauss--Legendre quadrature
(700 nodes; stability of results under variation to 500 and 900 nodes
was verified).
The tail norm is computed via the stable formula
\[
  \|(I-\PN)f_{mn}\|^2
  = \|f_{mn}\|^2
  - \sum_{k=0}^{N-1}
...
\]
which avoids catastrophic cancellation in the direct subtraction.
...
while for $m, n \approx N$ the tail remains of order one
and does not exhibit significant decay with $N$.
Intermediate (off-diagonal) pairs show intermediate behavior.

\begin{remark}
The near-zero tail values for bulk pairs
are consistent with strong numerical concentration
of PSWF products within $\PN$
in the bulk regime.
We note that machine-precision saturation
is expected for the bulk from spectral concentration
theory (\cite{OsipovRokhlinXiao}, Ch.~4--5)
and does not by itself constitute a proof of exponential decay;
it is included here as an illustration of the regime structure.
\end{remark}
...
$\|(I-\PN)f_{mn}\|$ on the index distance $|m-n|$,
...
the qualitative structure posited in
...
The transition between bulk and edge regimes
appears numerically sharp rather than gradual,
suggesting that the effective spectral cutoff
is well localized near $N \sim c$.
...
This is precisely the qualitative regime structure
encoded in Conjecture~\ref{conj:pswf_product_tail},
and motivates the separation between spectral tail control
and quadrature stability in the overall framework.
...
%%============================================================
%% Figure stubs (uncomment and place figures in numerics/)
%%============================================================
...
%     $\|(I-\PN)f_{mn}\|$ vs.\ projection dimension $N$
%     for representative index pairs
%     (bulk, off-diagonal, edge)
%     at $c=50$ (left) and $c=100$ (right). Log scale.}
%   \label{fig:tail-vs-N}
% \end{figure}
...
%     vs.\ index $m$ at $N=c$,
%     for $c=50$ and $c=100$. Log scale.
%     Sharp transition near $m \approx N$ separates bulk from edge regime.}
%   \label{fig:diag-tail}
% \end{figure}
