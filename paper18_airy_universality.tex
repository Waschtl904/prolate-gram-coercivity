...
\title[Paper XVIII: Airy Universality]{%
  Paper~XVIII: Airy Kernel Universality (v22)\\[0.3em]
  \large Tensorised Parseval $\cdot$
  Slepian $\alpha$-Gluing $\cdot$
  Neutral Limit Phrasing}
...
\medskip
\textbf{v22: three sentence-level fixes to v21.}
\begin{enumerate}[nosep]
\item \textbf{Tensorised Parseval
  (Remark~\ref{rem:dct}, Step~3):}
  $L^2([0,M]^2)$-norm formula restated as
  projection onto $\mathrm{span}\{u_1,\dots,u_K\}$ in each
  variable separately in $L^2(\R_+)\otimes L^2(\R_+)$
  (RS II Thm.~II.10).
\item \textbf{Slepian $\alpha$-gluing explicit
  (Lemma~\ref{lem:slepianunif}):}
  inner ($\beta_1$) and deep-tail ($\beta_2$) exponents
  unified as $\alpha:=\min(\beta_1,\beta_2)>0$.
\item \textbf{Neutral phrasing
  (Remark~\ref{rem:iterated}):}
  ``non-uniformity is harmless''
  $\to$
  ``no joint uniform convergence in $(c,K)$
  is required for the diagonal limit argument''.
\end{enumerate}
\end{abstract}
...
Fix $\delta\in(0,\tfrac{1}{3})$.

\textbf{Inner transition regime} ($K_0\leq k\leq c^{1/3+\delta}$,
$c\geq c_1$).
Osipov--Rokhlin--Xiao~\cite{Osipov2013} \S4.2.1
yields $\lambda_{n_c+k}^{(c)}\leq e^{-\beta_1 k}$
with $\beta_1=\beta_1(\delta)>0$ uniform in $c\geq c_1(\delta)$.

\textbf{Deep tail} ($k>c^{1/3+\delta}$).
Slepian~\cite{Slepian1978} Thm.~1:
$\lambda_{n_c+k}^{(c)}<2e^{-c\,\gamma_0}$
for $\gamma_0=\gamma_0(\delta)>0$;
for $k>c^{1/3+\delta}$ this gives
$\lambda_{n_c+k}^{(c)}<2e^{-\beta_2 k}$
with $\beta_2=c^{2/3-\delta}\gamma_0\geq\beta_2(\varepsilon)>0$
for $c\geq c_1(\varepsilon)$ fixed.

\textbf{Gluing.}
Set $\alpha:=\min(\beta_1,\beta_2)>0$.
This single exponent satisfies~\eqref{eq:uniftail}
for all $k\geq K_0$ and $c\geq c_1$, covering both regimes.
\end{proof}
...
for the diagonal limit argument;}
only the sequential limit
(first $c\to\infty$ for fixed $K$, then $K\to\infty$) is used.
\end{remark}
...
\begin{remark}[DCT: Cauchy--Schwarz + Tonelli + tensorised Parseval]
\label{rem:dct}
...
\textit{Step 3: Tensorised Parseval in $L^2(\R_+)\otimes L^2(\R_+)$.}
The kernel has the expansion
$\cKA=\sum_k u_k\otimes u_k$ in
$L^2(\R_+)\otimes L^2(\R_+)$ (RS~\cite{ReedSimon2} Thm.~II.10).
Truncating to $\mathrm{span}\{u_1,\dots,u_K\}$
in each variable separately gives
\begin{equation}\label{eq:tensorparseval}
  \normh{\cKA-\SK}{L^2([0,M]^2)}^2
  = \sum_{j>K}
     \normh{u_j}{L^2([0,M])}^2\cdot
     \normh{u_j}{L^2([0,M])}^2
  \leq \sum_{j>K}\normh{u_j}{L^2([0,M])}^2
  \to 0
\end{equation}
by Bessel's inequality in $L^2(\R_+)$
(since $\sum_j\normh{u_j}{L^2([0,M])}^2
=\int_0^M\cKA(\xit,\xit)\,d\xit<\infty$
by~\eqref{eq:diagbound}).
Hence $\SK\to\cKA$ in $L^2([0,M]^2)$,
giving a subsequence converging a.e.\ on $[0,M]^2$;
monotonicity $\SK(\xit,\xit)\nearrow\cKA(\xit,\xit)$
(diagonal) then extends full-sequence a.e.\ convergence
off-diagonal via~\eqref{eq:CStail}.
...
Tonelli: $\normh{h}{L^1}=(\int\sqrt{\cdot}\,d\xit)^2$
  & \checkmark & Eq.~\eqref{eq:Tonelli} \\
Tensorised Parseval ($L^2(\R_+)\otimes L^2(\R_+)$)
  & \checkmark & Eq.~\eqref{eq:tensorparseval}, RS II.10 \\
Slepian $\alpha$-gluing (two regimes, one exponent)
...
\boxed{
  ...
  h\in L^1([0,M]^2)
  \;+\;
  \underbrace{\ell_{C0}}_{e^{-\alpha k},\,\alpha{=}\min(\beta_1,\beta_2)}
  \xrightarrow{\text{tensor Parseval}+\text{DCT}}
  \underbrace{m}_{\text{Main}}
  \to T
}
\]
