...
\title[Paper XVIII: Airy Universality]{%
  Paper~XVIII: Airy Kernel Universality (v23)\\[0.3em]
  \large Subsequence Principle $\cdot$ Tensor Projection Explicit}
...
\medskip
\textbf{v23: two sentence-level patches to Remark~\ref{rem:dct} Step~3.}
\begin{enumerate}[nosep]
\item \textbf{Subsequence principle} replaces
  the incorrect ``monotonicity of $\SK$ gives full-sequence a.e.''
  claim: $L^2([0,M]^2)$-convergence + uniqueness of limit
  $\Rightarrow$ full-sequence $L^2$, then subsequential a.e.
\item \textbf{Tensor projection made explicit:}
  $\SK=\sum_{j\leq K}(P_j\otimes P_j)$,
  $P_j=\langle u_j,\cdot\rangle u_j$ rank-1 in $L^2(\R_+)$;
  norm identity from orthogonality of the $P_j$
  in $L^2(\R_+)\otimes L^2(\R_+)$.
\end{enumerate}
\end{abstract}
...
for all $k\geq K_0$ and $c\geq c_1$.
\end{proof}
...
\textit{Step 3: Tensorised Parseval + $L^2$-convergence.}
Write
$\SK=\sum_{j=1}^K P_j\otimes P_j$
where $P_j=\langle u_j,\cdot\rangle u_j$
is the rank-1 projection in $L^2(\R_+)$.
Orthogonality of the $P_j$ in $L^2(\R_+)\otimes L^2(\R_+)$
(RS~\cite{ReedSimon2} Thm.~II.10) gives
...
Since the $L^2$-limit is unique (equal to $\cKA$),
the full sequence $\SK$ converges in $L^2([0,M]^2)$;
a standard subsequence extraction then yields
$\SK_n(\xit,\xit')\to\cKA(\xit,\xit')$
for a.e.\ $(\xit,\xit')\in[0,M]^2$,
which is the only property used in Step~4.

\textit{Step 4: Lebesgue DCT.}
Dominated bound~\eqref{eq:CStail} with $h\in L^1([0,M]^2)$
+ a.e.\ convergence (Step~3)
$\Rightarrow$ $\SK\to\cKA$ in $L^1([0,M]^2)$.
\emph{No trace of $\mathrm{Id}_{[0,M]}$;
no HS-norm of the full kernel;
no Mercer monotonicity;
no monotonicity of $\SK$ in $K$.}
\end{remark}
...
Tonelli: $\normh{h}{L^1}=(\int\sqrt{\cdot})^2$ explicit
  & \checkmark & Eq.~\eqref{eq:Tonelli} \\
Tensor: $\SK=\sum P_j\otimes P_j$, norm via orthogonality
  & \checkmark & Eq.~\eqref{eq:tensorparseval} \\
$L^2$-convergence $\Rightarrow$ subsequential a.e.
  & \checkmark & Rem.~\ref{rem:dct} Step~3 \\
No monotonicity of $\SK$ used
  & \checkmark & Rem.~\ref{rem:dct} Step~4 note \\
Slepian: two regimes, $\alpha=\min(\beta_1,\beta_2)$
  & \checkmark & Lem.~\ref{lem:slepianunif} \\
Gluing covers $k\geq K_0$ without gap
  & \checkmark & Lem.~\ref{lem:slepianunif} proof \\
...
\[
\boxed{
  \underbrace{\SK=\textstyle\sum_{j\leq K}P_j{\otimes}P_j}_{\text{tensor proj.}}
  \xrightarrow{L^2}
  \cKA
  \xrightarrow{\text{subseq. a.e.}+h\in L^1}
  \underbrace{\text{DCT on }[0,M]^2}_{\ell_D}
  \to m\to T
}
\]
