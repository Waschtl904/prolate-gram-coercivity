...
\title[Paper XVIII: Airy Universality]{%
  ...
  \large \textsc{Final Version} --- BR3 Closed}
...
\textit{Step 3: $L^2$-convergence via Bessel.}
Write $\SK=\sum_{j=1}^K P_j\otimes P_j$
where $P_j=\langle u_j,\cdot\rangle u_j$
is the rank-1 projection in $L^2(\R_+)$.
Since the $P_j$ are orthogonal in $L^2(\R_+)$
(RS~\cite{ReedSimon2} Thm.~II.10),
\begin{equation}\label{eq:tensorparseval}
  \normh{\cKA-\SK}{L^2([0,M]^2)}^2
  \leq \normh{\cKA-\SK}{L^2(\R_+^2)}^2
  = \sum_{j>K}\normh{u_j}{L^2(\R_+)}^4 = \sum_{j>K} 1
\end{equation}
--- but this does not converge directly.
Instead, use Bessel's inequality in $L^2(\R_+)$
applied to the restriction:
\begin{equation}\label{eq:besseltail}
  \normh{\cKA-\SK}{L^2([0,M]^2)}^2
  \leq \sum_{j>K}\normh{u_j}{L^2([0,M])}^2
  \leq \int_0^M\cKA(\xit,\xit)\,d\xit
  - \sum_{j\leq K}\normh{u_j}{L^2([0,M])}^2
  \to 0
\end{equation}
as $K\to\infty$
(since $\int_0^M\cKA(\xit,\xit)\,d\xit<\infty$
by~\eqref{eq:diagbound}
and the left partial sums are monotone non-decreasing).
Hence $\SK\to\cKA$ in $L^2([0,M]^2)$;
subsequential a.e.\ convergence follows by standard extraction.
...
Bessel tail $\leq$ via restriction
  & \checkmark & Eq.~\eqref{eq:besseltail} \\
$L^2 \Rightarrow$ subsequential a.e.
  & \checkmark & Rem.~\ref{rem:dct} Step~3 \\
Slepian: $\alpha=\min(\beta_1,\beta_2)$, two regimes
  & \checkmark & Lem.~\ref{lem:slepianunif} \\
Iterated limit, no joint $(c,K)$ uniformity
  & \checkmark & Rem.~\ref{rem:iterated} \\
Langer ($c^{-2/3}$ window, uniform $j\leq K$)
