...
Consequently, the conditional implication of \cite{PaperI} applies
...
Consequently, Assumption~3.10 of \cite{PaperI} holds conditionally
...
required by \cite{PaperI} without the additional structural input
of Conjecture~\ref{conj:pswf_product_tail}(ii).
...
by Assumption~3.10 of \cite{PaperI}.
...
the algebraic defect profile required in Assumption~3.10 of \cite{PaperI}.
\end{remark}
...
\[
  |E_{mn}|\le C_p(1+|m-n|)^{-p}+Cc^{-1/3}.
\]
Hence Assumption~3.10 of \cite{PaperI} holds conditionally for
...
\ref{conj:xry_stability} are established, the compatibility
...
quadrature compatibility argument; see Problem~5 of \cite{PaperIII}.
\end{problem}
