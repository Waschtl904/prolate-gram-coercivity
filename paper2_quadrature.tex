...
do not assume any closure of the PSWF system under pointwise
multiplication, and we do not identify XRY with any spectral projector.
The substantive output is a clean separation between the PSWF
product-tail issue, the XRY stability issue, and the downstream
frame-theoretic consequences. More precisely, the only step in the present
argument that is not reducible to a perturbation analysis around the
projector $\PN$ is the spectral control of the nonlinear PSWF product
family; once that input is combined with the XRY stability statement, the
frame-theoretic consequences follow from a perturbative analysis.
