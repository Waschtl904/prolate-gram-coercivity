...
product-tail conjecture and an XRY spectral stability conjecture. Their
combination yields a conditional verification mechanism for the compatibility
assumption used in \cite{PaperI}.}
...
conjectural inputs. First, one needs a PSWF product-tail statement for the
analytic product family
...
inputs, namely a PSWF product-tail conjecture and an XRY spectral stability
conjecture on the relevant product family
...
components:
\begin{enumerate}[(i)]
\item \emph{PSWF product-tail conjecture.} The product integrands
...
condition of \cite{PaperI}. We deliberately avoid any claim that PSWF
product spaces are closed under pointwise multiplication, and we do not
identify XRY with any spectral projector. The substantive output is a clean
separation between the PSWF product-tail issue, the XRY stability issue,
and the downstream frame-theoretic consequences.

\subsection*{Main result}

\begin{theorem}[Conditional implication theorem]\label{thm:main_informal}
Assume Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability},
...
This paper should be read as a conditional framework paper rather than a
Theorem-paper in the traditional sense. Its purpose is to isolate the two
core unresolved analytical components that stand between the abstract
framework of \cite{PaperI} and a concrete verification for XRY PSWF-Gauss
quadrature. The PSWF product-tail step and the XRY stability step are both
left as explicit conjectural inputs. What the present analysis contributes
is the exact implication chain showing how these two inputs together force
the compatibility condition and hence the frame-theoretic conclusions.
...
%% ==================================================================
\section{PSWF Product-Tail Conjecture}\label{sec:spectral_decay}
%% ==================================================================
...
\begin{conjecture}[PSWF product-tail conjecture]\label{conj:pswf_tail}
For the family
\[
  \mathcal{F}_N:=\{\psi_m^{(c)}\overline{\psi_n^{(c)}}:0\le m,n\le N\},
\]
there exist constants $C,\alpha>0$ and a sequence $\varepsilon_N\downarrow0$
such that, for every $f_{mn}\in\mathcal{F}_N$,
\[
  \|(I-\PN)f_{mn}\|_{L^2([-T,T])}
  \le C e^{-(\alpha-\varepsilon_N)N}
  \qquad(0\le m,n\le N\sim c).
\]
This conjecture encapsulates the full PSWF-side spectral tail input needed
in the sequel. Its verification appears to be a central open analytical
problem in the present framework.
\end{conjecture}
...
Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability} isolate the
two unresolved analytical inputs used in this paper. Accordingly, the
present work should be read as an implication framework for the quadrature
compatibility problem of \cite{PaperI}, not as an unconditional verification
of that compatibility condition.
\end{remark}
...
Assume Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability},
together with Assumption~\ref{ass:pswf_sup}. Then for all $0\le m,n\le N-1$,
\[
...
\]
By Conjecture~\ref{conj:pswf_tail},
...
Assume Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability},
together with Assumptions~\ref{ass:pswf_sup}, \ref{ass:node_sep},
and~\ref{ass:weights}. Let $(\mathbf{p},\mathbf{w})$ be the XRY
PSWF-Gauss rule of order $N\sim c$. Then for every $p\ge1$ there exist
constants $C_p,C>0$ such that
\[
  |E_{mn}|\le C_p(1+|m-n|)^{-p}+Cc^{-1/3}
  \qquad(0\le m,n\le N-1).
\]
...
once Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability} are
...
Assume Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability},
together with Assumption~\ref{ass:pswf_sup}. Then there exists $C_*>0$ such
that
\[
...
Assume Conjectures~\ref{conj:pswf_tail} and~\ref{conj:xry_stability},
together with Assumptions~\ref{ass:pswf_sup}, \ref{ass:node_sep},
and~\ref{ass:weights}. Let $(\mathbf{p},\mathbf{w})$ be the XRY PSWF-Gauss
rule of order $N=\gamma c$ with $\gamma<2/\pi$ in the bulk regime. Once
...
The defect $\EN$ is controlled by two conjectural inputs: the PSWF
product-tail conjecture and the XRY stability conjecture. Under these
inputs, the abstract conditional framework of \cite{PaperI} becomes
...
\begin{problem}[Derive the PSWF product-tail conjecture]
Prove Conjecture~\ref{conj:pswf_tail} rigorously, with explicit constants
and explicit dependence on $m$, $n$, $c$, and $T$.
\end{problem}
