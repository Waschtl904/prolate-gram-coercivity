...
the two scenarios:
compute $\log |\Dc(1)| = \sum_k \log \varepsilon_k^{(c)}$
numerically for $c \in \{10, 50, 100, 200\}$
and check whether the growth is $O(c \log c)$
(exponential, scenario~B) or $O(c^{\alpha})$
for $\alpha < 1$ (subexponential, possibly scenario~A).
This single numerical experiment would resolve
the dichotomy with high confidence.
