...
The inputs are: the Landau--Widom eigenvalue distribution
\cite{Widom1964}, and the bandwidth doubling obstruction
of Paper~III \cite{PaperIII}.
...
\begin{equation}\label{eq:omega_nstar}
  ...
  \;\ge\; c_0(\delta,c)\,\|\psi_{n^*}^{(c)}\|_{L^2} > 0.
\end{equation}
Equivalently, as a supremum over unit vectors in $\Pedge$:
\begin{equation}\label{eq:omega_op}
  \|(I-\PN)\Kop\Pedge\|_2
  = \sup_{\substack{f\in V_{N,c},\,\|f\|=1\\\Pedge f = f}}
    \|(I-\PN)\Kop f\|
  \;\ge\; c_0(\delta,c) > 0.
\end{equation}
\end{enumerate}
...
By the Landau--Widom theorem \cite{Widom1964}, for $N = \lfloor\alpha c\rfloor$
with $\alpha < 2/\pi$:
\[
...
\]
which is $\sim C(\alpha)c$ with $C(\alpha) = 2(1-\alpha\pi/2)/\pi > 0$
(since $\alpha < 2/\pi$). The bulk modes $n \le (1-\delta)N$ contribute
at most $O(e^{-\eta_\delta c})$ to this sum (exponential concentration
of $\lambda_n$ near 1 in the bulk, Landau--Widom). Therefore
\[
  \sum_{n=(1-\delta)N}^{N-1}(1-\lambda_n(c))
  \ge C(\alpha)c - O(e^{-\eta_\delta c}) - (1-\delta)N\cdot 0
  \ge C'(\alpha,\delta)c > 0,
\]
and dividing by $\delta N \sim \delta\alpha c$ gives
$c_1(\delta,c) = C'(\alpha,\delta)/(\delta\alpha) > 0$.

\textbf{(b) Existence of an unstable direction.}
From part~(a), there exists at least one $n^*\in[(1-\delta)N,N)$
with $1-\lambda_{n^*}(c) \ge c_1(\delta,c)/2 > 0$.
...
(Paper~III, Lemma~3.1; Proposition~\ref{lem:slepian_cite} below):
$\Eout(|\psi_{n^*}^{(c)}|^2)
= \sum_{k\ge N}(1-\lambda_k)|\ip{\psi_{n^*}}{\psi_k}|^2$,
and since $|\ip{\psi_{n^*}}{\psi_k}|^2$ are the squared expansion
coefficients of $\psi_{n^*}$ in the PSWF basis:
\[
  \|(I-\PN)\Kop\psi_{n^*}\|^2
  = \sum_{k\ge N}\lambda_k^2|\ip{\psi_{n^*}}{\psi_k}|^2
  \ge (\inf_{k\ge N}\lambda_k)^2\cdot\Eout(|\psi_{n^*}|^2)
  \ge (\inf_{k\ge N}\lambda_k)^2\cdot c_0^{\mathrm{III}}.
\]
Since all prolate eigenvalues $\lambda_k(c) > 0$ and
$\inf_{k\ge N}\lambda_k(c) =: \lambda_{\min}(N,c) > 0$
(positivity of prolate spectrum, \cite{Slepian1978}), we obtain
$\|(I-\PN)\Kop\psi_{n^*}\| \ge c_0(\delta,c)\|\psi_{n^*}\| > 0$
with $c_0 := \lambda_{\min}\sqrt{c_0^{\mathrm{III}}}$.
The operator norm bound~\eqref{eq:omega_op} follows since
the norm is a supremum and $\psi_{n^*}/\|\psi_{n^*}\|$
is a unit vector in $\Pedge$.
\end{proof}

\begin{remark}[What Lemma~$\Omega$ says and does not say]%
\label{rem:omega}
\textbf{What it says.}
The PSWF concentration operator $\Kop$ has a non-trivial unstable
direction in the edge window: there exists a single mode $\psi_{n^*}$
that leaks significantly above index $N$.
The norm $\|(I-\PN)\Kop\Pedge\|_2$ witnesses this as a
\emph{supremum over directions}, not a uniform property of all modes.

\textbf{What it does not say.}
Lemma~$\Omega$ does \emph{not} assert that every mode in the edge
window is unstable, nor that
$\inf_{n\ge(1-\delta)N}(1-\lambda_n) > 0$ uniformly.
The edge window is a transition regime: eigenvalues range from
near~1 (at index $(1-\delta)N$) to near~0 (at index $N-1$).
Part~(a) controls the \emph{average} defect; part~(b) extracts
a single unstable direction. That is sufficient for Theorem~B.

\textbf{Arithmetic independence.}
The proof uses only Landau--Widom \cite{Widom1964} and
Paper~III \cite{PaperIII}. No sampling, no primes, no PNT, no GRH.
The constants $c_0, c_1$ are intrinsic spectral constants of
the prolate system.
\end{remark}
...
irreducibly. Lemma~$\Omega$(b) is the operator-theoretic
formulation of this phase transition: it identifies the direction
$\psi_{n^*}$ along which the collapse is most visible.
\end{remark}
...
\[
  ...
  \ge \tilde c_0 > 0,
\]
where $\tilde c_0 = c_0(\delta,c)\lambda_{\min}(N,c)/4$
...
\begin{enumerate}[\rm(1)]
  ...
  $\|(I-\PN)\Kop\psi_{n^*}\| \ge c_0\|\psi_{n^*}\| > 0$.
...
where $\tilde c_0$ is the constant of Theorem~B
(hence of Lemma~$\Omega$).
...
The constant $\tilde c_0$ in all five parts is determined by
the spectral constants $c_0(\delta,c)$, $c_1(\delta,c)$ of
Lemma~$\Omega$. It is intrinsic to the PSWF concentration operator
$\Kop$ and \textbf{independent of any sampling measure or
arithmetic input}.
\end{theorem}

\begin{proof}
\textbf{(i)} Pointwise limit exists as before (reproducing kernel
bound). Bulk--edge decomposition: $\Tcal$ is the bulk limit
(Theorem~\ref{thm:bulk_funct}); $\Flim^{\partial}$ the pointwise
limit of $\Delta^{\mathrm{arith}}$ (Proposition~\ref{prop:arith_defect}).

\textbf{(ii)} Any $\Kop$-commuting $A$ is PSWF-diagonal, so
$\ip{Af}{f} = \sum_n a_n|c_n|^2$ represents a form whose
associated measure $\nu_A$ is absolutely continuous with respect
to $\rhocl\,dx$ (Paper~IV \cite{PaperIV}). But
$\muarith\perp\rhocl\,dx$. Contradiction.

\textbf{(iii)} By Theorem~\ref{thm:edge_obstruction}:
$\norm{\Flim^{\partial}}_2
\ge\liminf_N\norm{\Fcal-\Tcal}_2\ge\tilde c_0>0$.

\textbf{(iv)} $\Flim^S = \Tcal$ (Route~S takes $c\to\infty$
first, eliminating $\Delta^{\mathrm{arith}}$).
$\Flim^A = W(h)$ (Route~A takes $N\to\infty$ first,
preserving $\muarith\to$ Weil measure).
Difference $\ge\tilde c_0$ by (iii).

\textbf{(v)} Triangle inequality at $\Tcal' = \Tcal$. \hfill$\square$
\end{proof}
...
\begin{enumerate}[(I)]
  ...
  Lemma~$\Omega$):
  average defect $\frac{1}{\delta N}\sum_{n\ge(1-\delta)N}(1-\lambda_n)
  \ge c_1 > 0$; $\exists\,n^*$ with
  $\|(I-\PN)\Kop\psi_{n^*}\|\ge c_0\|\psi_{n^*}\|>0$;
  ...
  $\liminf_N\norm{\Fcal(\QM^{\mathrm{arith}})-\Tcal}_2
  \ge\tilde c_0>0$;
  arithmetic realizes Lemma~$\Omega$ via the direction $n^*$.
...
Characterize all $n^*\in[(1-\delta)N,N)$ satisfying
$\|(I-\PN)\Kop\psi_{n^*}\|\ge c_0\|\psi_{n^*}\|$.
Is the maximizer of $\|(I-\PN)\Kop\psi_n\|$ unique? Does it
concentrate near the Airy transition point?
\end{problem}

\begin{problem}\label{prob:omega_sharp}
Determine the sharp value of $c_0(\delta,c)$ in Lemma~$\Omega$(b).
Is it monotone in $\delta$? Does it diverge as $\alpha\to 2/\pi$?
\end{problem}

\begin{problem}\label{prob:lw_mass_sharp}
Determine the sharp constant $c_1(\delta,c)$ in Lemma~$\Omega$(a).
By Landau--Widom, $c_1 \sim (1-\alpha\pi/2)/(\delta\alpha)$ as
$c\to\infty$; verify the sub-leading term.
\end{problem}
...
\medskip
\noindent\textbf{Step~1: Fix an unstable direction (Lemma~$\Omega$).}

By Lemma~\ref{lem:omega}(b), there exists
$n^* \in [(1-\delta)N, N)$ such that
\begin{equation}\label{eq:nstar_bound}
  \|(I-\PN)\Kop\psi_{n^*}^{(c)}\|
  \ge c_0(\delta,c)\,\|\psi_{n^*}^{(c)}\| > 0,
\end{equation}
with $c_0(\delta,c)$ independent of $N$ and of any arithmetic data.
Set $e^* := \psi_{n^*}^{(c)}/\|\psi_{n^*}^{(c)}\|$ (unit vector).

\medskip
\noindent\textbf{Step~2: Translate to the defect form.}

Let $D_{N,c} := \Fcal(\QM^{\mathrm{arith}}) - \Tcal$.
By Proposition~\ref{prop:arith_defect}:
\[
  \ip{D_{N,c} e^*}{e^*}
  = \underbrace{\ip{\Delta^{\mathrm{quad}}e^*}{e^*}}_{=\,(E_{N,c})_{n^*n^*}}
  + \underbrace{\ip{\Delta^{\mathrm{weight}}e^*}{e^*}}_{=\,O(\log N/\sqrt{N})}.
\]

\medskip
\noindent\textbf{Step~3: Lower bound on $(E_{N,c})_{n^*n^*}$.}

By the Slepian energy identity
(Prop.~\ref{lem:slepian_cite}):
$\sum_{k\ge N}(1-\lambda_k)|a_k^{n^*}|^2 = \Eout(|\psi_{n^*}|^2)
\ge c_0^{\mathrm{III}}(\delta,c) > 0$
(bandwidth doubling, Prop.~\ref{prop:bwdoubling_cite}, valid since
$n^*\ge(1-\delta)N$).
In particular $\sum_{k\ge N}|a_k^{n^*}|^2 \ge c_0^{\mathrm{III}}$.
Since $\lambda_{n^*}(c)\to 0$ (Landau--Widom, $n^*$ is in the
tail of the spectrum for $\alpha<2/\pi$):
\[
  (E_{N,c})_{n^*n^*}
...
\medskip
\noindent\textbf{Step~5: Operator norm lower bound via $n^*$.}
...
\[
  \|D_{N,c}\|_2 \ge \ip{D_{N,c}e^*}{e^*} \ge \tilde c_0
\]
uniformly for $N\ge N_0$, so
$\liminf_N\|\Fcal(\QM^{\mathrm{arith}})-\Tcal\|_2
\ge\tilde c_0 > 0$. \hfill$\square$

\begin{remark}[Structure of the proof]
The direction $e^* = \psi_{n^*}/\|\psi_{n^*}\|$ is the
``witness'' for the obstruction. It is selected in Step~1 from
Lemma~$\Omega$(b) using only spectral data.
Arithmetic (PNT) enters only in Step~4, to show that the weight
correction is negligible. The constant $\tilde c_0 = c_0^{\mathrm{III}}/4$
is determined entirely by the spectral witness and is independent
of primes.
\end{remark}
...
for $N$ sufficiently large (depending on $\delta,c$).
\end{proposition}
